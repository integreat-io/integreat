...
const Source = require('./integreat/source')
const ItemMapper = require('./integreat/source/itemMapper')
const ValueMapper = require('./integreat/source/valueMapper')

import Integreat from './_integreat'
...
const createSource = () => {
  ...
  source.endpoints.all = 'http://some.api/entries/'
...
test('sources should exist', (t) => {
  ...
  t.is(great.sources.constructor.name, 'Map')
})

test.todo('should add type to sources')
...
test('should sync source', async (t) => {
  ...
  const dispatch = sinon.stub(great, 'dispatch')
...
test.skip('should emit sync event after syncing source', async (t) => {
  const great = new Integreat()
  ...
  t.truthy(items.length, 1)
  t.truthy(items[0])
  t.is(items[0].id, 'item1')
})

test.skip('should schedule next sync after syncing source', async (t) => {
  const great = new Integreat()
  const source = createSource()
...
test('should set mappers on sources on start', async (t) => {
  ...
  const payload = schedule.args[0][1]
  const item = payload.itemMappers.entry
...
test('should set filters on sources on start', async (t) => {
  ...
  const payload = schedule.args[0][1]
  const item = payload.itemMappers.entry
  t.is(item.filters.from.length, 1)
  t.is(item.filters.from[0], filter)
})
...
test('should sync sources on start', async (t) => {
  ...
  const sourceDef = {
    id: 'entry1',
    adapter: 'entries',
    endpoints: {all: 'http://some.api/entries/'},
...
test.skip('should call actionHandler with action and sources', async (t) => {
  const items = [{id: 'ent1', type: 'entry'}]
  const source = {
    getEndpoint: (key) => (key === 'one') ? 'http://api.test/entry:ent1' : null,
    fetchItems: () => items
  }
  const great = new Integreat()
  great.sources.set('entries', source)
  const action = {type: 'GET', source: 'entries', payload: {id: 'ent1', type: 'entry'}}
