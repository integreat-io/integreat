...
test('should get access from shortcut', (t) => {
  const access = 'auth'
  const actionType = 'GET'
  const expected = { allow: 'auth' }
...
test('should only allow accepted methods', (t) => {
  t.deepEqual(accessForAction('all')('GET'), { allow: 'all' })
  t.deepEqual(accessForAction('auth')('GET'), { allow: 'auth' })
  t.deepEqual(accessForAction(undefined)('GET'), {})
  t.deepEqual(accessForAction('none')('GET'), { allow: 'none' })
  t.deepEqual(accessForAction('illegal')('GET'), { allow: 'none' })
