...
test('should set new entry with raw data as user', async (t) => {
  ...
  const action = {
    type: 'SET',
    payload: { type: 'entry', data: putData, params: { rawForAll: true } },
...
test('should set new entry with raw data as root', async (t) => {
  ...
  const action = {
    type: 'SET',
    payload: { type: 'entry', data: putData2, params: { rawForRoot: true } },
...
test('should return error when user is setting new entry with raw data', async (t) => {
  ...
  const action = {
    type: 'SET',
    payload: { type: 'entry', data: putData3, params: { rawForRoot: true } },
    meta: { ident: { id: 'johnf', roles: ['editor'] } },
  }
