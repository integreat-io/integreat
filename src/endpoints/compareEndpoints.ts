...
const hasParams = (endpoint, required) =>
  !!endpoint.params &&
  // eslint-disable-next-line security/detect-object-injection
  Object.keys(endpoint.params).filter(key => endpoint.params[key] === required)
    .length
const hasFilters = endpoint =>
  Array.isArray(endpoint.filters) && endpoint.filters.length
