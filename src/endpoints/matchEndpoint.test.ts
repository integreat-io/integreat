...
const filterNotDraft = ({ data } = {}) => data && data.draft === false
const filterEntry1 = ({ data } = {}) => data && data.title === 'Entry 1'
const filterParamAuthor = ({ params } = {}) =>
  params && params.author === 'johnf'
const filterMetaRootIdent = ({ meta } = {}) =>
  meta && meta.ident && meta.ident.root
...
const endpointPostWithAuthor = {
  match: { action: 'POST', params: { author: true } }
}
const endpointGetWithOptionalAuthor = {
  match: { action: 'GET', params: { author: false } }
}
const endpointPostWithFilter = {
  match: { action: 'POST', filters: [filterNotDraft] }
}
const endpointPostWithFilters = {
  match: { action: 'POST', filters: [filterNotDraft, filterEntry1] }
}
const endpointPostWithNoFilters = { match: { action: 'POST', filters: [] } }
const endpointPostWithParamFilter = {
  match: { action: 'POST', filters: [filterParamAuthor] }
}
const endpointPostWithMetaFilter = {
  match: { action: 'POST', filters: [filterMetaRootIdent] }
}
...
test('should match when required params is present', t => {
  const endpoints = sort([
    endpointGet,
    endpointPostWithAuthor,
    endpointGetWithAuthor
  ])
