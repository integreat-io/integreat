...
test('loadSourceDefsFromDb should set source defs', (t) => {
  ...
  const sourceDefs = [
    {id: 'source:src1', type: 'source', itemtype: 'entry', adapter: 'json', _key: 'entry'},
    {id: 'source:src2', type: 'source', itemtype: 'account', adapter: 'ad', _key: 'account'}
  ]
  ...
  .then(() => {
    const src1 = great.getSource('entry')
    t.truthy(src1)
    t.is(src1.adapter, 'json')
    const src2 = great.getSource('account')
    t.truthy(src2)
...
test('should schedule sources on start', (t) => {
  ...
  // Set sources
  const sourceDef = {itemtype: 'entry', adapter: 'entries', sync: {schedule: 3600}}
...
test('should set mappers on sources on start', (t) => {
  ...
  .then(() => {
    const payload = schedule.args[0][1]
    t.is(payload.fetch.map.length, 1)
    t.is(payload.fetch.map[0], mapper)
  })
})
...
test('should set filters on sources on start', (t) => {
  ...
  .then(() => {
    const payload = schedule.args[0][1]
    t.is(payload.fetch.filter.length, 1)
    t.is(payload.fetch.filter[0], filter)
  })
})
