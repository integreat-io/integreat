...
test('loadSourceDefsFromDb should set source defs', (t) => {
  ...
  const sourceDefs = [
    {id: 'source:src1', type: 'source', itemtype: 'entry', sourcetype: 'json', _key: 'entry'},
    {id: 'source:src2', type: 'source', itemtype: 'account', sourcetype: 'ad', _key: 'account'}
  ]
  ...
  .then(() => {
    ...
    t.truthy(src1)
    t.is(src1.sourcetype, 'json')
    const src2 = great.getSource('account')
    t.truthy(src2)
    t.is(src2.sourcetype, 'ad')
  })
})
