...
test('should set source with id from sourcetype', (t) => {
  const great = new Integreat()
  const source = {sourcetype: 'source'}

  great.setSource(source)
  const ret = great.getSource('source')
...
test('getStorage should return storage', (t) => {
  ...
  t.deepEqual(ret._db.config, config)
})
...
test('should start server and return promise of server instance', (t) => {
  const great = new Integreat()
