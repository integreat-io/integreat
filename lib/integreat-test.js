...
test.skip('should provide worker with dispatch method', async (t) => {
  const params = {}
  const action = {
    type: 'RUN',
    payload: {
...
test.skip('should queue scheduled actions', async (t) => {
  const great = integreat({sources, datatypes}, {adapters})
...
test.skip('should queue from schedule defintion object', async (t) => {
  const great = integreat({sources, datatypes}, {adapters})
  sinon.stub(great, 'queue').resolves({status: 'queued'})
