...
test('should set rev for one item', async (t) => {
  ...
  const endpoints = {
    getRevs: {uri: 'http://some.couchdb.com/_all_docs?keys=[%22{ids}%22]', path: 'rows[].value.rev'}
  }
  ...
  const expectedReq = {uri: 'http://some.couchdb.com/_all_docs?keys=[%22entry%3Aent1%22]'}
...
test('should set rev for more items', async (t) => {
  ...
  const endpoints = {
    getRevs: {
      uri: 'http://some.couchdb.com/_all_docs?keys=[%22{ids}%22]',
      path: 'rows[].value.rev'
    }
  }
  const source = createSource({id: 'entries', endpoints, adapter: json})
  ...
  const expectedReq = {
    uri: 'http://some.couchdb.com/_all_docs?keys=[%22entry%3Aent1,entry%3Aent3,entry%3Aent2%22]'
  }
