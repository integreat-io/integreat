...
test('middleware should return response with status ok when queuedStatus is ok', async (t) => {
  const queue = setupQueue(mockQueue())
  const dispatch = sinon.stub().resolves({ status: 'ok' })
  const action = {
    type: 'GET',
    payload: { type: 'entry' },
    meta: { queue: true, queuedStatus: 'ok' },
  }
  const expected = { status: 'ok', data: { id: 'queued1' } }
...
test('should throw when queue.subscribe rejects', async (t) => {
  const dispatch = sinon.stub().resolves({ status: 'ok' })
  const mock = mockQueue()
  sinon.stub(mock, 'subscribe').rejects('')

  const queue = setupQueue(mock)
  await t.throwsAsync(queue.setDispatch(dispatch))
})
