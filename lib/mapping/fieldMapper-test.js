...
test('fromSource should return value from param', (t) => {
  ...
  const request = {params: {isActive: true}}
  const target = {}
  const expected = {attributes: {active: true}}

  const ret = attr.fromSource({}, target, request)

  t.deepEqual(ret, expected)
})

test('fromSource should return value from request', (t) => {
  const attr = fieldMapper({key: 'source', param: '$source'})
  const request = {params: {}, source: 'entries'}
  const target = {}
  const expected = {attributes: {source: 'entries'}}
...
test('toSource should set rel on target according to path', (t) => {
  const attr = fieldMapper({key: 'author', path: 'meta.writer'}, {isRelationship: true})
  const data = {relationships: {author: {id: 'johnf', type: 'user'}}}
