...
const wrapResponse = (data, params) => data // ({data, params})
...
test.skip('fromService should use array of paths as alternative paths', (t) => {
  const def = {
    type: 'entry',
    attributes: { one: ['values.first', 'values.second'] }
  }
  const mapping = setupMapping({ schemas })(def)
  const data1 = wrapResponse({ values: { first: 1 } })
  const data2 = wrapResponse({ values: { second: 2 } })
...
test.skip('fromService should use param instead of attribute path', (t) => {
  const def = {
    type: 'entry',
    attributes: { one: '$params.first' }
  }
  const mapping = setupMapping({ schemas })(def)
  const data = wrapResponse({ values: { first: 1 } }, { first: 10 })
...
test.skip('fromService should use param instead of relationship path', (t) => {
  const def = {
    type: 'entry',
    relationships: {
  ...
  const mapping = setupMapping({ schemas })(def)
  const data = wrapResponse({ item: { notes: 'no1' } })
