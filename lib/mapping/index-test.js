...
test('fromService should map full relationship item', (t) => {
  ...
  const data = wrapData({ item: { writer: { username: 'johnf', fullname: 'John F.' } } })
  const expected = { id: 'johnf', type: 'user', attributes: { name: 'John F.' }, relationships: {} }
...
test('fromService should map array of full relationship items', (t) => {
  ...
  const mapping = setupMapping({ schemas, mappings })(def)
  const data = wrapData({ item: { notes: [
    { id: 'note1', note: 'Note 1.' },
    { id: 'note2', note: 'Note 2.' }
  ] } })
  const expected = [
    { id: 'note1', type: 'comment', attributes: { text: 'Note 1.' }, relationships: {} },
    { id: 'note2', type: 'comment', attributes: { text: 'Note 2.' }, relationships: {} }
  ]
