...
test('fromService should format attributes with named formatters', (t) => {
  const def = {
    type: 'entry',
    attributes: {
      one: {
        path: 'values.first',
        format: ['addTwo', 'square']
      }
    }
  }
  const formatters = {
  ...
  const mapping = setupMapping({ schemas, formatters })(def)
  const data = wrapResponse({ values: { first: 1 } })
...
test('fromService should use formatter with relationship', (t) => {
  const def = {
    type: 'entry',
    relationships: {
      author: { path: 'user', format: 'removeDomain' }
    }
  }
  const formatters = {
  ...
  const mapping = setupMapping({ schemas, formatters })(def)
  const data = wrapResponse({ user: 'desk/johnf' })
