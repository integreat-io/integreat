...
test('should set id, type, and service', (t) => {
  const mapping = setupMapping({id: 'entriesMapping', type: 'entry', service: 'entries'}, {schemas})

  t.truthy(mapping)
  t.is(mapping.id, 'entriesMapping')
  t.is(mapping.type, 'entry')
  t.is(mapping.service, 'entries')
  t.is(mapping.schema, schemas.entry)
})

test('should set default values', (t) => {
  const mapping = setupMapping({type: 'entry'}, {schemas})

  t.is(mapping.id, null)
  t.is(mapping.service, null)
})
...
test('fromService should exist', (t) => {
  ...
  t.is(typeof mapping.fromService, 'function')
})
...
test('fromService should return empty array when no data', (t) => {
  ...
  const mapping = setupMapping(def, {schemas})
...
test('fromService should return several items', (t) => {
  ...
  const item = setupMapping(def, {schemas})
...
test('fromService should not map type from service', (t) => {
  const def = {
    type: 'entry',
...
test('fromService should use array of paths as alternative paths', (t) => {
  ...
  const [ret1] = mapping.fromService(data1)
  const [ret2] = mapping.fromService(data2)
...
test('fromService should use param instead of attribute path', (t) => {
  ...
  const [ret] = mapping.fromService(data, params)
...
test('fromService should use transform pipeline', (t) => {
  ...
  const mapping = setupMapping(def, {schemas, transformers})
...
test('fromService should provide transform function with original data', (t) => {
  const def = {
    type: 'entry',
    attributes: {title: 'title'},
    transform: [
...
test('toService should return undefined when no data', (t) => {
  const mapping = setupMapping({type: 'entry'}, {schemas})
...
test('toService should provide transform function with original data', (t) => {
  const def = {
    type: 'entry',
    attributes: {one: {path: 'values.first'}},
    transform: [{
...
test('toService should return null when filter returns false', (t) => {
  ...
  const ret = mapping.toService(data, {old: true})
