...
test('fromSource should use param instead of attribute path', (t) => {
  ...
  const request = {params: {first: 10}}

  const [ret] = mapping.fromSource(data, request)
...
test('fromSource should use param instead of relationship path', (t) => {
  ...
  const request = {params: {comment: 'no3'}}

  const [ret] = mapping.fromSource(data, request)

  t.deepEqual(ret.relationships.comments, 'no3')
})
