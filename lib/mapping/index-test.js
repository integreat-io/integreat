...
test('fromService should return several items', (t) => {
  ...
  const data = wrapData({ items: [{ values: { first: 1 } }, { values: { first: 2 } }] })
...
test('fromService should use mutate pipeline', (t) => {
  ...
  const mapping = setupMapping({ schemas, mutators })(def)
...
test('fromService should still mutate when no attr/rel mappers', (t) => {
  const schemas = { other: schema({ id: 'other', attributes: { title: 'string' } }) }
  const def = {
    type: 'other',
    mutate: [
      (item) => ({ ...item, attributes: { title: 'Mutated!' } })
    ]
  }
  const mapping = setupMapping({ schemas })(def)
  ...
  t.is(ret.id, 'item')
  t.is(ret.attributes.title, 'Mutated!')
})
...
test('fromService should not map with toService mappings', (t) => {
  ...
  const data = {
    items: [
      { key: 'ent1', values: { first: 1, section: 'news' }, refs: { notes: ['comment1'] } },
      { key: 'ent2', values: { first: 2, section: 'news' }, refs: { notes: ['comment2'] } }
    ]
  }
  const params = { section: 'news' }
  const expected = [
    { id: 'ent1', type: 'entry', attributes: { one: 1 }, relationships: { comments: [{ id: 'comment1', type: 'comment' }] } },
    { id: 'ent2', type: 'entry', attributes: { one: 2 }, relationships: { comments: [{ id: 'comment2', type: 'comment' }] } }
  ]
...
test('toService should transform relationships', (t) => {
...
test('toService should mutate item', (t) => {
  const def = {
    type: 'entry',
    attributes: { id: 'key' },
    mutate: ['first']
...
test('toService should mutate item with mutateTo', (t) => {
  const def = {
    type: 'entry',
    attributes: { id: 'key' },
    mutateTo: ['first']
...
test('toService should use mutate when no attr/rel mappers', (t) => {
  const def = {
    type: 'entry',
    mutate: [
      revFn((item) => ({ ...item, attributes: { title: 'From mutate' } }))
    ]
  }
  const mapping = setupMapping({ schemas })(def)
  ...
  t.is(ret.id, 'item')
  t.is(ret.attributes.title, 'From mutate')
})
