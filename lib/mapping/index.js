...
function prepareMapping ({ id = null, type = null, ...attributes }, relationships, toService, transformers) {
  if (hasFieldMappings(id, attributes, relationships)) {
    ...
    return {
      ...
      relationships: map(normalize, relationships),
...
function mapping ({
  ...
  return (mapping = {}, overrideType) => {
    ...
    const transformFwdPipeline = transformPipeline.map(
      compose(
        fwd,
        transform
      )
    ...
    const filterFromPipeline = preparePipeline(filterFrom, filters).map(
      compose(
        fwd,
        filter
      )
    )
    const filterToPipeline = preparePipeline(filterTo, filters).map(
      compose(
        rev,
        filter
      )
