...
function mapping ({
  ...
  return {
      ...
      return items
        .map((data) => {
          ...
          const item = (mappings.length > 0)
            ? mappings.reduce((target, field) => field.fromSource(data, target, request), {type})
            : data
    ...
    toSource (data, target = {}) {
      if (!filterWithFilters(data, filterToPipeline)) {
        return null
      }

      let item = (mappings.length > 0)
        ? mappings.reduce((target, field) => field.toSource(data, target), {})
        : data

      item = mapWithMappers(item, transformPipeline, /* reverse */ true, data)

      return setPath(target, path, item)
    }
  }
}
