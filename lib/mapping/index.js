...
const prepareMapping = ({ id = null, type = null, ...attributes }, relationships, transformers) =>
...
const prepareRevPipeline = (mutateToDef, mutatePipeline, mutators) => (mutateToDef)
  ? preparePipeline(mutateToDef, mutators).map(compose(rev, transform))
  : mutatePipeline.map((fn) => (fn.rev) ? rev(transform(fn.rev)) : null).filter(Boolean)
...
const mapping = ({ mutators, filters, transformers, schemas = {} } = {}) => ({
  ...
  const def = [
    ...
    prepareMapping(attrDefs, relDefs, transformers),
    fwd(transform(schema.cast)),
    ...mutateFwdPipeline,
    ...mutateRevPipeline,
    ...filterFromPipeline,
    ...filterToPipeline,
