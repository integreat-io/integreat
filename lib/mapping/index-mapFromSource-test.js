...
test('should map attributes and relationships from param', async (t) => {
  const data = {
  ...
  const src = source({id: 'entries', adapter, mappings}, {datatypes})
  const params = {title: 'Any item', $source: 'entries'}

  const ret = await src.mapFromSource(data, 'entry', {params})

  t.is(ret[0].attributes.name, 'Any item')
  t.deepEqual(ret[0].relationships.source, {id: 'entries', type: 'source'})
})
...
test('should include all values from type', async (t) => {
  ...
  const ret = await src.mapFromSource(data, 'entry', {useDefaults: true})
...
test('should not include all values from type', async (t) => {
  ...
  const ret = await src.mapFromSource(data, 'entry', {useDefaults: false})
...
test('should filter items with itemDef', async (t) => {
  ...
  const datatypes = {entry: datatype({id: 'entry'})}
  const src = source({id: 'entries', adapter, mappings}, {datatypes})
...
test('should return data with asterisk item', async (t) => {
  ...
  t.is(ret[0].attributes.title, 'Other item')
})
...
// All other meta cases are handled for mapToSource
test('should provide meta mapping when handling meta', async (t) => {
  ...
  t.is(ret[0].attributes.lastSyncedAt.getTime(), lastSyncedAt)
})
