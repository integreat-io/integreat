...
const idFromRel = (rel) => (rel)
  ? (Array.isArray(rel)) ? rel.map((val) => val.id) : rel.id
  : undefined

const setFieldValue = (target, key, value, isRelationship) => {
...
} = {},
{
  ...
  return {
     ...
     */
    fromSource (data, target, request = {}) {
      if (key === 'type') {
        return target
      }
      const value = (param) ? getParam(request, param) : getPath(data, path)
      if (value === undefined) {
        return target
      }

      const mappedValue = mapWithMappers(value, formatPipeline, /* reverse */ false, value)
      return setFieldValue(target, key, mappedValue, isRelationship)
    },
