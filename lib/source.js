...
class Source {
  ...
  getEndpoint (key, params) {
    const template = this.endpoints[key]
    return (template) ? parseUriTemplate(template, params) : null
  }
  ...
  async fetchItems (endpoint) {
    ...
    const data = await this.adapter.retrieve(endpoint, this.auth)
  ...
  async sendItems (data, endpoint) {
    ...
    return await this.adapter.send(endpoint, item)
  }
}
