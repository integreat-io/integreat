...
function source (
  ...
  return {
    ...
    prepareRequest (request) {
      const {action, type, headers = {}, uri, params = {}} = request
      const {options: endpoint} = getEndpoint(endpoints, request) || {}
      const typePlural = plurals[type] || (type && `${type}s`)
    ...
    async retrieve (request, {useDefaults = false} = {}) {
      const types = [].concat(request.type)
      if (!request.type || types.length === 0) {
      ...
      if (ret.status === 'ok') {
        ret.data = (await Promise.all(
          types.map((type) => {
            if (mappings[type]) {
              return mappings[type].fromSource(ret.data, request)
                .map((item) => mappings[type].datatype.cast(item, {useDefaults}))
            }
          })
        ))
          .reduce(flatten, [])
    ...
    async send (request, {useDefaults = false} = {}) {
      const typed = [].concat(request.data)
        .map((item) => mappings[item.type] && mappings[item.type].datatype.cast(item, {useDefaults}))

      const data = typed
        .map((item) => item && mappings[item.type] && mappings[item.type].toSource(item))
        .filter((item) => item !== undefined)

      const response = await this.sendSerialized({
        ...request,
        data: (Array.isArray(request.data)) ? data : (data[0])
      })

      return (response.status === 'ok') ? {...response, data: typed} : response
    }
  }
}
