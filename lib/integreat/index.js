...
},
{
  ...
  const great = {
    ...
    async queue (action, timestamp = null) {
      ...
      if (typeof pushToQueue !== 'function') {
        throw new Error('No queue is provided')
      }

      try {
        const id = await pushToQueue(action, timestamp)
  ...
  if (queue && typeof queue.subscribe === 'function' && typeof queue.push === 'function') {
    queue.subscribe(great.dispatch)
    pushToQueue = queue.push.bind(queue)
  }
