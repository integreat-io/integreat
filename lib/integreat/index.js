...
function integreat (sourceDefs, typeDefs, resources = {}) {
  ...
  const sourceArr = sourceDefs.map((def) => source(def.id, prepare(def, resources)))
  const sources = sourceArr.reduce(reduceToObject('id'), {})

  const types = typeDefs.reduce(reduceToObject('id'), {})
