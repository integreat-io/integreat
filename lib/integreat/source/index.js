...
function source (
  ...
  return {
     ...
     * @returns {Object} The retrieved data
     */
    async retrieveNormalized ({endpoint, params = {}, method = 'GET', data}) {
      let request = {method, data}
      try {
        request = Object.assign(request, this.getEndpoint(endpoint, params) || {})
      } catch (error) {
      ...
      if (response.status === 'ok') {
        try {
          const data = await adapter.normalize(response.data, request.path)
