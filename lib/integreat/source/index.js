 ...
 * @param {Object} params - Object with params adapter, endpoints, baseUri, mappings, and auth
 * @returns {Object} The created source
 */
function source (id, {adapter, auth = null, baseUri = null, endpoints = {}, mappings = {}} = {}) {
  if (!adapter) {
    throw new TypeError(`Can't create source '${id}' without an adapter.`)
  }

  const itemMappers = Object.keys(mappings)
    .map((type) => itemMapper(mappings[type]))
