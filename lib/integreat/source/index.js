...
function source (
  ...
  beforeRetrieve = lookup(beforeRetrieve, hooks)
  afterRetrieve = lookup(afterRetrieve, hooks)
  afterNormalize = lookup(afterNormalize, hooks)
  beforeSerialize = lookup(beforeSerialize, hooks)
  beforeSend = lookup(beforeSend, hooks)
  afterSend = lookup(afterSend, hooks)
  ...
  return {
    ...
    async retrieveRaw (uri) {
      const request = {uri}
      if (typeof beforeRetrieve === 'function') {
        await beforeRetrieve(request, {source: this})
      }

      let response
      try {
        response = await adapter.retrieve(request.uri, auth)
      } catch (error) {
        response = createError(`Error retrieving from ${uri}. ${error}`)
      }

      if (typeof afterRetrieve === 'function') {
        await afterRetrieve(response, {source: this})
      }
      return response
    },
    ...
    async retrieveNormalized ({endpoint: endpointId, params = {}}) {
      ...
      if (typeof afterNormalize === 'function') {
        afterNormalize(response, {source: this})
      }
    ...
    async sendRaw (uri, data, method) {
      ...
      if (typeof beforeSend === 'function') {
        await beforeSend(request, {source: this})
      }

      let response
      try {
        response = await adapter.send(request.uri, request.data, auth, request.method)
      } catch (error) {
        response = createError(`Error sending to ${uri}. ${error}`)
      }

      if (typeof afterSend === 'function') {
        await afterSend(response, {source: this})
      }
      return response
    },
     ...
     * @returns {Object} Response object with status and data
     */
    async sendSerialized ({endpoint, params = {}, data}) {
      const request = {data}
      try {
        Object.assign(request, this.getEndpoint(endpoint, params) || {})
      } catch (error) {
        return createError(`Cannot send to enpoint ${endpoint} on source '${id}'. ${error}`)
      }
      ...
      if (typeof beforeSerialize === 'function') {
        await beforeSerialize(request, {source: this})
      }

      try {
        data = await adapter.serialize(request.data, request.path)
      } catch (error) {
        return createError(`Error mapping data from endpoint '${endpoint}' on source '${id}'. ${error}`)
      }
      return this.sendRaw(request.uri, data, request.method)
    },
