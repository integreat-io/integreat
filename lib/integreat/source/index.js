const {generate: generateUri} = require('great-uri-template')
...
function source (
  ...
  endpoints = Object.keys(endpoints)
    .reduce((object, key) => ({
      ...object,
      [key]: adapter.prepareEndpoint(endpoints[key], {baseUri})
    }), {})
  ...
  return {
    ...
    getEndpoint (key, params) {
      ...
      if (ret && ret.uri) {
        return {...ret, uri: generateUri(ret.uri, params)}
      }
      return ret || null
    },
    ...
    async retrieveRaw (request) {
      ...
      let response
      try {
        response = await adapter.send(Object.assign({auth, method: 'GET'}, request))
      } catch (error) {
     ...
     * @returns {Object} The retrieved data
     */
    async retrieveNormalized ({endpoint, params = {}, method, data}) {
      let endpointOptions
      try {
        endpointOptions = this.getEndpoint(endpoint, params)
      } catch (error) {
        return createError(`Cannot retrieve from endpoint '${endpoint}' on source '${id}'. ${error}`)
      }

      if (!endpointOptions) {
        return createError(`Cannot retrieve from unknown endpoint '${endpoint}' on source '${id}'.`)
      }

      const request = {
        ...endpointOptions,
        method: endpointOptions.method || method || 'GET',
        data
      }
      ...
      if (response.status === 'ok') {
        try {
          const normalized = await adapter.normalize(response.data, request.path)
          response = Object.assign({}, response, {data: normalized})
        } catch (error) {
     ...
     */
    async retrieve ({endpoint, params = {}, type, useDefaults = false, method, data}) {
      const types = [].concat(type)
      if (!type || types.length === 0) {
        return {status: 'ok', data: []}
      }

      const ret = await this.retrieveNormalized({endpoint, params, method, data})
     ...
     * @returns {Object} Response object with status and data
     */
    async sendSerialized ({endpoint, params = {}, data, method}) {
      let endpointOptions
      try {
        endpointOptions = this.getEndpoint(endpoint, params)
      } catch (error) {
      ...
      const request = {
        ...endpointOptions,
        method: endpointOptions.method || method || 'PUT',
        data
      }
      ...
      let serialized
      try {
        serialized = await adapter.serialize(request.data, request.path)
      } catch (error) {
