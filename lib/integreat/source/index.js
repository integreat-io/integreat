...
function source (
  ...
  return {
     ...
     */
    async mapFromSource (data, type, {path = null, params = {}, useDefaults = false} = {}) {
      if (type && data) {
        const mapper = itemMappers[type] || itemMappers['*']
        if (mapper) {
          const items = await adapter.normalize(data, path)
     ...
     */
    async mapToSource (data, path = null, {useDefaults = false} = {}) {
      const mapOne = (data) => {
        const {type} = data || {}
        if (type) {
          const mapper = itemMappers[type] || itemMappers['*']
     ...
     */
    async retrieve ({endpoint: endpointId, params = {}, type, useDefaults = false}) {
      ...
      if (ret.status === 'ok') {
        ...
        try {
          const itemsPerTypes = await Promise.all(
            types.map((type) => source.mapFromSource(ret.data, type, {path, params, useDefaults}))
          )
     ...
     * @returns {Object} Response object with status and data
     */
    async send ({endpoint: endpointId, params = {}, data, useDefaults = false}) {
      let endpoint
      try {
        endpoint = this.getEndpoint(endpointId, params) || {}
      } catch (error) {
      ...
      let mappedData
      try {
        mappedData = await this.mapToSource(data, path, {useDefaults})
      } catch (error) {
        return createError(`Error mapping data from endpoint '${endpointId}' on source '${id}'. ${error}`)
      }
