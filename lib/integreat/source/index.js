...
const compilePath = require('../../utils/path/compile')
...
function source (
  ...
  const itemMappers = Object.keys(mappings)
    .map((type) => [Object.assign({type}, mappings[type]), datatypes[type]])
      ...
      if (ret.status === 'ok') {
        ...
        try {
          const itemsPerTypes = await Promise.all(
            types.map((type) => source.mapFromSource(ret.data, type, {path, mappedValuesOnly}))
          )
