...
class Source {
   ...
   * @params {Object} key - The key for the uri template endpoint
   * @params {Object} params - Object with key and value for the expansion
   * @returns {string} Endpoint uri
   */
  getEndpoint (key, params) {
    const ret = {uri: null}
    const {uri: template} = this.endpoints[key] || {}
    if (template !== null && template !== undefined) {
      ret.uri = parseUriTemplate((this.baseUri || '') + template, params)
    }
    return ret
  }
  ...
  async mapFromSource (data, type = null) {
    ...
    const processOne = async (def) => {
      ...
      return [].concat(items || [])
        .map((item) => def.fromSource(item))
        .filter((item) => def.filterFromSource(item))
    }

    if (type && this.itemMappers[type]) {
      return processOne(this.itemMappers[type])
    } else {
      const itemDefs = Object.values(this.itemMappers)
  ...
  async mapToSource (data) {
    ...
    const itemDef = this.itemMappers[data.type]
    if (itemDef) {
      const item = itemDef.toSource(data)
      if (itemDef.filterToSource(item)) {
        return await this.adapter.serialize(item, itemDef.path)
      } else {
        return null
      }
    }
   ...
   */
  async fetchItems (uri) {
    if (!this.adapter || !uri) {
      return []
    }
    const data = await this.adapter.retrieve(uri, this.auth)
   ...
   * @param {string} uri - The enpoint url to fetch from
   * @returns {Promise} Promise that will be resolved when item is sent
   */
  async sendItems (data, uri) {
    if (!this.adapter || !data || !uri) {
      return null
    }
    const item = await this.mapToSource(data)
    return await this.adapter.send(uri, item)
  }
}
