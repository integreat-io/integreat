...
test('fromSource should use updatedAt when createdAt is not set', (t) => {
  ...
  const attributes = [{key: 'updatedAt', type: 'date', path: 'updatedAt'}]
  const item = itemMapper({type: 'entry', attributes})
...
test('fromSource should include restValues', (t) => {
  const data = {values: {first: 1, second: {value: 2}}}
  const attributes = [
    {key: 'one', type: 'integer', path: 'values.first'},
    {key: 'two', type: 'integer', path: 'values.second.value'}
  ...
  const item = itemMapper({type: 'entry', attributes, restValues})
...
test('fromSource should not include restValues', (t) => {
  const data = {values: {first: 1, second: {value: 2}}}
  const attributes = [
    {key: 'one', type: 'integer', path: 'values.first'},
    {key: 'two', type: 'integer', path: 'values.second.value'}
...
test('fromSource should use transform pipeline', (t) => {
  ...
  const item = itemMapper({type: 'entry', attributes, transform})
...
test('fromSource should use data as item if asterisk type', (t) => {
  const data = {
    id: 'item',
    type: 'other',
    createdAt: new Date(),
...
test('fromSource should set dates for asterisk type', (t) => {
  const before = Date.now()
...
test('fromSource should use transform on asterisk type', (t) => {
  const data = {id: 'item', type: 'other'}
  const transform = [
...
test('toSource should map attributes', (t) => {
  ...
  const attributes = [{key: 'one', type: 'integer', path: 'values.first', format}]
  const item = itemMapper({type: 'entry', attributes})
...
test('toSource should map relationships', (t) => {
  ...
  const relationships = [{key: 'comments', type: 'comment', path: 'item.note', format}]
  const item = itemMapper({type: 'entry', relationships})
...
test('toSource should map item', (t) => {
  const data = {id: 'ent1', type: 'entry', attributes: {one: 1}}
  const attributes = [{key: 'one', type: 'integer', path: 'values.first'}]
  const transform = [{to: (item) => Object.assign({}, item, {title: 'First was here'})}]
  const item = itemMapper({type: 'entry', attributes, transform})
...
test('toSource should use data as item if asterisk type', (t) => {
  const data = {
    id: 'item',
    type: 'other',
    createdAt: new Date(),
...
test('toSource should use transform', (t) => {
  const data = {id: 'item', type: 'other'}
  const transform = [
