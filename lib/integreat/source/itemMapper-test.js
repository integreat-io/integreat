...
test('fromSource should generate random id', (t) => {
  const dtype = datatype({id: 'entry', attributes: {title: 'string'}})
  const def = {type: 'entry', path: 'item', attributes: {title: {}}}
  const item = itemMapper(def, {datatype: dtype})
...
test('fromSource should map attributes for item', (t) => {
  ...
  const item = itemMapper(def, {datatype: type})
...
test('fromSource should use array attribute def as path', (t) => {
  const data = {values: {second: 2}}
  const type = datatype({attributes: {one: 'integer'}})
  const def = {
    type: 'entry',
...
test('fromSource should use alternative attribute path', (t) => {
  ...
  const def = {
    type: 'entry',
    attributes: {one: {path: ['values.unknown', 'values.first']}}
  }
  const type = datatype({attributes: {one: 'integer'}})
  const item = itemMapper(def, {datatype: type})
...
test('fromSource should use createdAt and updatedAt attributes as dates for item', (t) => {
  const createdDate = new Date('2016-11-01')
  const updatedDate = new Date('2016-11-13')
  const data = {createdAt: createdDate, updatedAt: updatedDate}
  const def = {
    type: 'entry',
    attributes: {
...
test('fromSource should use createdAt when updatedAt is not set', (t) => {
  const createdDate = new Date('2016-11-01')
  const data = {createdAt: createdDate}
  const def = {
    type: 'entry',
    attributes: {createdAt: {path: 'createdAt'}}
  }
...
test('fromSource should use updatedAt when createdAt is not set', (t) => {
  const updatedDate = new Date('2016-11-01')
  const data = {updatedAt: updatedDate}
  const def = {
    type: 'entry',
    attributes: {updatedAt: {path: 'updatedAt'}}
  }
...
test('fromSource should always map id, createdAt, and updatedAt', (t) => {
  const createdDate = new Date('2017-02-27T12:00:00Z')
  const updatedDate = new Date('2017-03-05T18:00:00Z')
  const data = {id: 'ent1', createdAt: createdDate, updatedAt: updatedDate}
...
test('fromSource should include all values from type', (t) => {
  ...
  const {one, two, three} = ret.attributes
  t.is(one, 1)
  t.is(two, 2)
  t.is(three, 3)
...
test('fromSource should not include all values from type', (t) => {
  ...
  t.false(ret.attributes.hasOwnProperty('three'))
...
test('fromSource should provide transform function with original data', (t) => {
  const data = [{id: 'item1', title: 'First item', notMapped: 'Not mapped'}]
  const attributes = {id: 'id', name: 'title'}
  const type = datatype({attributes: {id: {}, name: {}}})
  const transform = [
    (item, data) => ({...item, notMapped: data.notMapped})
  ]
  const item = itemMapper({type: 'entry', attributes, transform}, {datatype: type})

  const ret = item.fromSource(data)

  t.is(ret[0].notMapped, 'Not mapped')
})
...
test('fromSource should qualify array of data', (t) => {
  const dtype = datatype({id: 'entry', attributes: {title: 'title'}})
  const qualifier = 'type="entry"'
  const attributes = {id: 'id', title: 'title'}
  const item = itemMapper({type: 'entry', qualifier, attributes}, {datatype: dtype})
  const data = [
...
test('toSource should map createdAt and updatedAt', (t) => {
  ...
  const data = {id: 'item1', type: 'entry', attributes: {createdAt, updatedAt}}
  const attributes = {
...
test('toSource should provide transform function with original data', (t) => {
  const data = {id: 'ent1', type: 'entry', attributes: {one: 1, notMapped: 'Not mapped'}}
  const attributes = {one: {path: 'values.first'}}
  const type = datatype({attributes: {one: {}}})
  const transform = [{
