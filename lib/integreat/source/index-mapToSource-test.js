...
import {set as setPath, get as getPath} from '../../utils/path'
...
const adapter = {
  serialize: async (data, path) => (path) ? setPath({}, path, data) : data,
  normalize: async (source, path) => (path) ? getPath(source, path) : source
}
...
test('should include all default values from type', async (t) => {
  ...
  const ret = await src.mapToSource(data, 'entry', {useDefaults: true})
