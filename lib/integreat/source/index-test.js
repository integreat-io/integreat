...
test('retrieveNormalized should add endpoint to request', async (t) => {
  const request = {action: 'GET', endpoint: 'one', id: 'ent1', type: 'entry', source: 'entries'}
  ...
  const expected = {
    ...
    id: 'ent1',
    source: 'entries',
    endpoint: {uri: ['http://some.api/1.0'], method: null, path: null},
    method: 'GET'
  }
...
test('sendSerialized should send to endpoint', async (t) => {
  ...
  const request = {action: 'SET', type: 'entry', source: 'entries', id: null, data}
