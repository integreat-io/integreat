...
test('getEndpoint should expand and return endpoint', (t) => {
  const source = new Source()
  source.endpoints.all = {uri: 'http://api.test/entries{?first,max}'}

  const {uri} = source.getEndpoint('all', {first: 11, max: 20})
...
test('getEndpoint should return null for unknown endpoint', (t) => {
  ...
  const {uri} = source.getEndpoint('unknown', {first: 11, max: 20})
...
test('getEndpoint should return endpoint with baseUri', (t) => {
  ...
  source.endpoints.one = {uri: '{type}:{id}'}

  const {uri} = source.getEndpoint('one', {id: 'ent1', type: 'entry'})
