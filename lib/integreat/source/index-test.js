...
test('retrieve should map data', async (t) => {
  ...
  const items = [{
    type: 'entry',
    path: 'data',
    attrs: [{key: 'id', path: 'key'}, {key: 'title', path: 'header'}]
  }]
  const src = source('entries', {endpoints, items, adapter: json})
  sinon.stub(src, 'retrieveRaw').returns(Promise.resolve(data))
  ...
  t.is(ret[0].attributes.title, 'The heading')
})
...
test('retrieve should use endpoint path', async (t) => {
  ...
  const items = [{type: 'entry', path: 'data', attrs: [{key: 'id', path: 'key'}]}]
  const src = source('entries', {endpoints, items, adapter: json})
  sinon.stub(src, 'retrieveRaw').returns(Promise.resolve(data))
