...
test('retrieveRaw should allow beforeRetrieve hook to alter request', async (t) => {
  ...
  await src.retrieveRaw('http://other.api/1.0/')

  t.is(retrieve.callCount, 1)
  t.is(retrieve.args[0][0], uri)
})
...
test('retrieveRaw should get beforeRetrieve hook by id', async (t) => {
  ...
  await src.retrieveRaw('http://some.api/1.0/')
...
test('sendRaw should allow beforeSend hook to alter request', async (t) => {
  ...
  await src.sendRaw('http://other.api/1.0', {}, 'PUT')

  t.is(send.callCount, 1)
  t.is(send.args[0][0], uri)
  t.is(send.args[0][1], data)
  t.is(send.args[0][3], method)
})
...
test('sendRaw should get beforeSend hook by id', async (t) => {
  ...
  await src.sendRaw('http://some.api/1.0/', {})
...
test('sendRaw should allow afterSend hook to alter response', async (t) => {
  ...
  const ret = await src.sendRaw('http://some.api/1.0/', {})
...
test('sendSerialized should send to endpoint', async (t) => {
  ...
  t.deepEqual(src.sendRaw.args[0][1], expected)
})

test('sendSerialized should send to endpoint with POST method', async (t) => {
  const data = {id: 'ent1', type: 'entry'}
...
test('sendSerialized should allow beforeSerialize hook to alter request', async (t) => {
  ...
  t.is(src.sendRaw.callCount, 1)
  t.is(src.sendRaw.args[0][0], 'http://other.api/1.0/entry:ent1')
  t.truthy(src.sendRaw.args[0][1])
  t.is(src.sendRaw.args[0][1].item, data)
  t.is(src.sendRaw.args[0][2], 'PUT')
})
