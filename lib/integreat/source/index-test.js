...
test('retrieve should map data', async (t) => {
  ...
  const src = source({id: 'entries', endpoints, mappings, adapter: json}, {types})
  sinon.stub(src, 'retrieveRaw').returns(Promise.resolve({status: 'ok', data}))
...
test('retrieve should include all values from type', async (t) => {
  const data = {data: [{id: 'ent1'}]}
  const endpoints = {all: {uri: 'http://some.api/1.0'}}
  const mappings = {entry: {path: 'data'}}
  const types = {entry: datatype({id: 'entry', attributes: {byline: {default: 'Somebody'}}})}
  const src = source({id: 'entries', endpoints, mappings, adapter: json}, {types})
  sinon.stub(src, 'retrieveRaw').returns(Promise.resolve({status: 'ok', data}))
...
test('retrieve should return error when mapFromSource rejects', async (t) => {
  ...
  const src = source({id: 'entries', endpoints, mappings, adapter}, {types})
