...
test('fromSource should format value with one format function', (t) => {
  const format = (value) => value.length
...
test('fromSource should map with array of format functions', (t) => {
  const format = [
    (value) => value.length,
    {from: (value) => value + 1},
    (value) => 'Result: ' + value
