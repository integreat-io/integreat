...
const setAttribute = (data) => (attributes, {key, path, fromSource}) => {
  if (key !== 'type') {
...
const setRelationship = (data) => (relationships, {key, type, path, fromSource}) => {
  const value = getPath(data, path)
...
const assembleValueObject = (key, valueType, valueMapping) => Object.assign(
  {key, type: valueType.type, default: valueType.default}, valueMapping
)
...
function itemMapper ({
  ...
  return {
    ...
    fromSource (data, {mappedValuesOnly = false} = {}) {
      ...
      if (type !== '*') {
        const defaultAttrs = (mappedValuesOnly) ? {} : Object.assign({}, restAttrs)
        const defaultRels = (mappedValuesOnly) ? {} : Object.assign({}, restRels)

        const attributes = attrMappers.reduce(setAttribute(data), defaultAttrs)
        const relationships = relMappers.reduce(setRelationship(data), defaultRels)
    ...
    toSource (data) {
      ...
      if (type === '*') {
        ...
        attrMappers.forEach(({key, path, toSource}) => {
          ...
          setPath(item, path, toSource(value))
        })
