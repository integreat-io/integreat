...
function itemMapper ({
  ...
  const attrMappers = createValuesFromParams(attrMappings)
  const relMappers = createValuesFromParams(relMappings)
    ...
    fromSource (data, {mappedValuesOnly = false} = {}) {
      ...
      if (type !== '*') {
        ...
        item = {
          ...
          relationships: mapRelationships(relMappers, data, defaultRels)
        }
      }
      ...
      const reverse = false
      return mapWithMappers(item, transform, reverse)
    },
