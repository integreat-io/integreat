...
test('should return error when push queue rejects', async (t) => {
  const queue = {
    subscribe: () => {},
    push: sinon.stub().rejects(new Error('Queue trouble'))
  }
  const action = {type: 'RUN'}
...
test('should throw when no queue is provided', (t) => {
  const great = integreat({sources, datatypes}, {adapters})
  const action = {type: 'RUN'}

  return great.queue(action)
    .then(
      () => { t.fail() },
      () => { t.pass() }
    )
