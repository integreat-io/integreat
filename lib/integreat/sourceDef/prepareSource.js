 ...
 * @param {Object} resources - Object with adapters, auths, transformers, filters, formatters, and types
 * @returns {Object} An object groomed and ready for the itemMapper function
 */
function prepareSource (def = {}, {adapters = {}, auths, transformers, filters, formatters, types = {}} = {}) {
  ...
  const createItem = (itemDef) => prepareItem(itemDef, {transformers, filters, formatters, typeDef: types[itemDef.type]})
  const mappings = Object.keys(def.mappings || {})
    .map((type) => Object.assign({type}, def.mappings[type]))
    .map(createItem)
