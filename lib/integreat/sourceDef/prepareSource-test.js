...
test('should create mappings array', (t) => {
  const sourceDef = {
    mappings: {entry: {path: 'the.path'}}
  }

  const ret = prepare(sourceDef)

  t.truthy(ret.mappings)
  const item = ret.mappings.entry
  t.truthy(item)
  t.is(item.type, 'entry')
  t.is(item.path, 'the.path')
})

test('should type mappings from type defs', (t) => {
  const attributes = [{key: 'id'}, {key: 'unknown'}, {key: 'age'}]
  const mappings = {entry: {attributes}}
  const sourceDef = {mappings}
  const types = {
    entry: {
      id: 'entry',
      attributes: {
  ...
  const {attributes: attrs} = ret.mappings.entry
