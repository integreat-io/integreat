...
test('should set defaultFrom from type def', (t) => {
  ...
  t.true(Array.isArray(ret.attrs))
  t.is(ret.attrs.length, 2)
  t.is(ret.attrs[0].defaultFrom, undefined)
  t.is(ret.attrs[1].defaultFrom, 23)
})
...
test('should only set defined attributes', (t) => {
  ...
  t.true(Array.isArray(ret.attrs))
  t.is(ret.attrs.length, 1)
  t.is(ret.attrs[0].key, 'title')
})
...
test('should always set id, createdAt, and updatedAt', (t) => {
  ...
  t.true(Array.isArray(ret.attrs))
  t.is(ret.attrs.length, 3)
  t.is(ret.attrs[0].key, 'id')
  t.is(ret.attrs[1].key, 'createdAt')
  t.is(ret.attrs[2].key, 'updatedAt')
})
...
test('should set relationship type and defaultFrom from type def', (t) => {
  ...
  t.is(ret.rels.length, 1)
  t.is(ret.rels[0].key, 'users')
  t.is(ret.rels[0].type, 'user')
  t.is(ret.rels[0].defaultFrom, 'admin')
})
