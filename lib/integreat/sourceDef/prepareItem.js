 ...
 * @param {Object} resources - Object with transformers, filters, formatters, and type
 * @returns {Object} An object groomed and ready for the itemMapper function
 */
function prepareItem (def, {transformers, filters, formatters, typeDef = {}} = {}) {
  const {
    type,
    path,
    attributes: attrDefs = [],
  ...
  const attributes = Object.keys(attrDefs)
    ...
    .map((attr) => prepareValue(attr, {formatters}))
  ...
  const relationships = Object.keys(relDefs)
    ...
    .map((rel) => prepareValue(rel, {formatters, isRel: true}))

  const {filterFrom = null, filterTo = null} = prepareFilters(filterDef, filters)
  const transform = preparePipeline(transformDef, transformers)
