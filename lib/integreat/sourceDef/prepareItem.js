...
function prepareItem (def, {mappers, filters, transforms, typeDef = {}} = {}) {
  ...
  const attributes = (attrDefs) ? attrDefs.filter(typedAttrsOnly).map(setAttrFromType).map(createAttr) : null
  ...
  const relationships = (relDefs) ? relDefs.filter(typedRelsOnly).map(setRelFromType).map(createRel) : null
