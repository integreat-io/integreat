...
test('should get metadata for source', async (t) => {
  nock('http://api1.test')
    ...
    .reply(200, {_id: 'entries', _rev: '000001', type: 'meta', attributes: {lastSyncedAt}})
  ...
  const expected = {source: 'entries', key: 'lastSyncedAt', value: lastSyncedAt}
...
test('should return null for metadata when not set on source', async (t) => {
  nock('http://api2.test')
    .get('/database/meta:entries')
    .reply(200, {_id: 'entries', _rev: '000001', type: 'meta'})
  ...
  const expected = {source: 'entries', key: 'lastSyncedAt', value: null}
