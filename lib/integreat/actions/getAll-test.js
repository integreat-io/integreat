...
function createSource (endpoint, {restValues = {}} = {}) {
  ...
  const mappings = {entry: {type: 'entry', attributes, restValues}}
  return source('entries', {adapter: couchdb, endpoints, mappings})
}
...
test('should get default values from type', async (t) => {
  ...
  const src = createSource('http://api1.test/database', {restValues})
  const sources = {entries: src}
