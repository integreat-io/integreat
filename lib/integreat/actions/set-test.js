...
const createSources = (endpoint) => {
  ...
  const src = source({id: 'entries', adapter: couchdb, mappings, endpoints}, {datatypes})
  return {entries: src}
}
...
test('should infere source id from type', async (t) => {
  ...
  const datatypes = {entry: datatype({source: 'entries'})}

  const ret = await set(payload, {sources, datatypes})
