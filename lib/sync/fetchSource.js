 ...
 * @param {Object} auth - An auth strategy object
 ...
 */
module.exports = function fetchSource (endpoint, path, adapter, auth, map, filter) {
  debug('Fetching from endpoint `%s`', endpoint)

  // Get source data
  return adapter.retrieve(endpoint, auth)
