...
const createSource = (adapter = null) => {
  ...
  attrs.push(new Attribute('id', null, 'id'))
  attrs.push(new Attribute('name', null, 'title'))
  attrs.push(new Attribute('createdAt', null, 'createdAt'))
  attrs.push(new Attribute('updatedAt', null, 'updatedAt'))
  return source
}
...
test('should retrieve, map, and return items with relationship', (t) => {
  ...
  source.item.relationships.push(new Attribute('comments', 'comment', 'comments'))
...
test('should filter items', (t) => {
  const source = createSource(mockdapter)
  source.item.filter.push((item) => item.id === 'item2')
