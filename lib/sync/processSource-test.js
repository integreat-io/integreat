...
test('should retrieve, map, and return items with relationship', (t) => {
  ...
  source.items.entry.relationships.push(new Attribute('comments', 'comment', 'comments'))
...
test('should map item with map pipeline', (t) => {
  ...
  source.items.entry.map.push((item) => {
    const attributes = Object.assign({}, item.attributes, {
...
test('should filter items', (t) => {
  const source = createSource(mockdapter)
  source.items.entry.filters.from.push((item) => item.id === 'item2')
...
test('should filter item through the filter pipeline', (t) => {
  const source = createSource(mockdapter)
  source.items.entry.filters.from.push((item) => true)
  source.items.entry.filters.from.push((item) => false)
  const expected = []
