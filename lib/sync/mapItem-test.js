...
test('should set createdAt and updatedAt to now when not specified', (t) => {
  ...
  t.is(ret.createdAt, ret.changedAt)
})

test('should use createAt and updateAt attributes as dates for item', (t) => {
  const createdAt = new Date('2016-11-01').getTime()
  const changedAt = new Date('2016-11-13').getTime()
  const item = {createdAt, changedAt}
  const attributes = {createdAt: {path: 'createdAt'}, changedAt: {path: 'changedAt'}}

  const ret = mapItem(item, {attributes})

  t.is(ret.createdAt, createdAt)
  t.is(ret.changedAt, changedAt)
  t.is(ret.attributes.createdAt, undefined)
  t.is(ret.attributes.changedAt, undefined)
})

test('should use createdAt when changedAt is not set', (t) => {
  const createdAt = new Date('2016-11-01').getTime()
