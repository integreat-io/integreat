...
const matchFilters = ({ match = {} }, { payload }) => !match.filters || match.filters.every((filter) => filter(payload))
...
function matchEndpoint (endpoints) {
  return ({ type, payload }) => endpoints
    .find((endpoint) =>
      ...
      matchFilters(endpoint, { type, payload }))
}
