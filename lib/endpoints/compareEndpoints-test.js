...
const endpointPostPutAndDelete = { match: { action: ['POST', 'PUT', 'DELETE'] } }
...
const endpointServiceAuthorParams = { match: { params: { service: true, author: false } } }
const endpointGetWithParam = { match: { action: 'GET', params: { author: true } } }
const endpointMemberWithParam = { match: { scope: 'member', params: { author: true } } }
const endpointEntryWithParam = { match: { type: 'entry', params: { author: true } } }
const endpointOneFilter = { match: { filters: [() => true] } }
const endpointMemberWithFilter = { match: { scope: 'member', filters: [() => true] } }
const endpointGetWithFilter = { match: { action: 'GET', filters: [() => true] } }
const endpointServiceParamWithFilter = { match: { params: { service: true }, filters: [() => true] } }
const endpointUserWithFilter = { match: { type: 'user', filters: [() => true] } }
const endpointEntryWithFilter = { match: { type: 'entry', filters: [() => true] } }
const endpointEntryWithTwoFilters = { match: { type: 'entry', filters: [() => true, () => false] } }
const endpointId = { id: 'endpoint1', match: { params: { includeDocs: false } } }
...
test('should sort more params before fewer', (t) => {
  const higher = compareEndpoints(endpointServiceAuthorParams, endpointAuthorParam)
  const equal = compareEndpoints(endpointServiceParam, endpointAuthorParam)
  const lower = compareEndpoints(endpointServiceParam, endpointServiceAuthorParams)
...
test('should sort required params before optional', (t) => {
  const requiredVsOptional = compareEndpoints(endpointServiceParamOptional, endpointAuthorParam)

  t.true(requiredVsOptional > 0)
})

test('should sort more filters before fewer', (t) => {
  const higher = compareEndpoints(endpointEntryWithTwoFilters, endpointEntryWithFilter)
  const equal = compareEndpoints(endpointEntryWithFilter, endpointUserWithFilter)
  const lower = compareEndpoints(endpointUserWithFilter, endpointEntryWithTwoFilters)
...
test('should sort filters after params', (t) => {
  ...
  const filterVsParam = compareEndpoints(endpointOneFilter, endpointServiceParam)
...
test('should sort with filters before without', (t) => {
  const typeWithVsWithout = compareEndpoints(endpointEntryWithFilter, endpointEntry)
  const paramWithVsWithout = compareEndpoints(endpointServiceParamWithFilter, endpointServiceParam)
  const scopeWithVsWithout = compareEndpoints(endpointMemberWithFilter, endpointMember)
  const actionWithVsWithout = compareEndpoints(endpointGetWithFilter, endpointGet)
