...
const getIndividualItems = async (ids, payload, ident, getSource) => {
  const responses = await Promise.all(ids.map((id) => get({payload: {...payload, id}, ident}, {getSource})))
...
async function get ({payload, ident}, {getSource} = {}) {
  ...
  if (Array.isArray(id) && !hasCollectionEndpoint(source.endpoints)) {
    return getIndividualItems(id, payload, ident, getSource)
  }
