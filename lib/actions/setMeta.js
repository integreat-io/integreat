...
async function setMeta ({ payload, ident }, { getService }) {
  ...
  const data = { id, type, attributes: meta }
  const action = {
    type: 'SET',
    payload: { keys: Object.keys(meta), type, id, data, endpoint },
    meta: { ident }
  }
  const { response } = await metaService.send(action, { onlyMappedValues: true })
  return response
}
