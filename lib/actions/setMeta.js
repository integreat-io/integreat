 ...
 */
async function setMeta (payload, {getSource, datatypes}) {
  ...
  if (!datatype) {
    debug(`SET_META: The datatype '${type}' was not found)`)
    return {status: 'error'}
  }
  ...
  const request = metaSource.prepareRequest({
    ...
    type,
    endpoint,
    params: {keys: Object.keys(meta), id},
    data: datatype.cast({id, type, attributes: meta}, {useDefaults: false})
  })
