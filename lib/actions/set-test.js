...
const setupService = (endpoint, { method = 'POST', path = 'docs[]', responseMapping, id = 'entries' } = {}) => {
  return createService({ schemas, setupMapping: setupMapping({ schemas, mappings }) })({
    id,
    adapter: json(),
    endpoints: [
      { requestMapping: path, responseMapping, options: { uri: endpoint, method } },
...
test('should authenticate items', async (t) => {
  ...
  const ret = await set({ type: 'SET', payload, meta: { ident } }, { getService, schemas })
...
test('should merge request data with response data', async (t) => {
  ...
  const src = setupService(
    'http://api9.test/database/_bulk_docs',
    { id: 'accounts', responseMapping: '.' }
  )
  const getService = (type, service) => (service === 'accounts') ? src : null
...
test('should allow null as request data', async (t) => {
  const scope = nock('http://api1.test')
    .post('/database/_bulk_docs', '{"docs":[null]}')
    .reply(201, [{ ok: true }, { ok: true }])
  const action = {
    type: 'SET',
    payload: {
