...
const createService = (endpoint, {endpointId = null, method = 'POST', path = 'docs[]', id = 'entries'} = {}) => {
  ...
  return setupService({id, adapter: json, endpoints}, {datatypes, mappings})
}
...
test('should map and set items to service', async (t) => {
  ...
  const ret = await set({payload}, {getService, datatypes})
...
test('should authenticate items', async (t) => {
  ...
  const ret = await set({payload, ident}, {getService, datatypes})
