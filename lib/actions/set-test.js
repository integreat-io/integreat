...
test('should map action data with origin source', async (t) => {
  const unmappedData = [{id: 'ent1', type: 'entry', title: 'Entry 1'}]
  const mappedData = [{id: 'ent1', type: 'entry', attributes: {title: 'Entry 1'}}]
  const payload = {
    type: 'entry',
    data: unmappedData,
    origin: 'api_json',
  ...
  const srcEntries = createSource('http://api1.test/database/_bulk_docs')
  const srcApi = {mapFromSource: sinon.stub().returns(mappedData)}
  sinon.stub(srcEntries, 'send').resolves({status: 'ok', data: [{}]})
  const getSource = (type, source) => (type === 'entry')
    ? srcEntries : (source === 'api_json') ? srcApi : null

  const ret = await set(payload, {getSource, datatypes})

  t.deepEqual(ret, {status: 'ok', data: mappedData})
  t.is(srcApi.mapFromSource.callCount, 1)
  t.deepEqual(srcApi.mapFromSource.args[0][0], unmappedData)
  t.deepEqual(srcApi.mapFromSource.args[0][1], {type: 'entry', useDefaults: true})
})

test('should not map action data with unmapped = false', async (t) => {
  const mappedData = [{id: 'ent1', type: 'entry', attributes: {title: 'Entry 1'}}]
  const payload = {
    type: 'entry',
    data: mappedData,
  ...
  t.is(ret.status, 'ok')
  t.is(srcApi.mapFromSource.callCount, 0)
})
