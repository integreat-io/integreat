...
async function set ({ payload, ident }, { getService, schemas }) {
  ...
  const { service: serviceId, data, endpoint, onlyMappedValues = true } = payload
  ...
  const action = {
    type: 'SET',
    payload: { ...payload, id, type },
    meta: { ident }
  }
  const { response, request } = await service.send(action, { onlyMappedValues })
