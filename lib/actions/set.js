...
async function set ({ payload, ident }, { getService, schemas }) {
  ...
  const { service: serviceId, data, params, endpoint, onlyMappedValues = true } = payload
  ...
  const { response, request: authorizedRequest } = await service.send(request, { onlyMappedValues })
