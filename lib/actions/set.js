...
const send = (payload, source) => {
  ...
  return source.send(request, {useDefaults})
}
...
async function set (payload, {getSource} = {}) {
  ...
  const {type, source: sourceId, data} = payload
  const source = (typeof getSource === 'function')
    ? getSource(type || data.type, sourceId) : null
