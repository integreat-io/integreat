...
function setupService(uri, match = {}, { id = 'entries' } = {}) {
  return createService({
    schemas,
    setupMapping: setupMapping({ schemas, mappings }),
  })({
    id,
    adapter: json(),
    endpoints: [
      { match, options: { uri, method: 'GET' } },
...
test('should get item by id in single data object from service', async (t) => {
  nock('http://api13.test')
    .get('/database/entry:ent1')
    .reply(200, { id: 'ent1', type: 'entry' })
  const payload = {
  ...
  const src = setupService('http://api13.test/database/{type}:{id}')
  const getService = (type, service) => (service === 'entries' ? src : null)
...
test('should get item by id in single data array from service', async (t) => {
  nock('http://api14.test')
    .get('/database/entry:ent1')
    .reply(200, { id: 'ent1', type: 'entry' })
  const payload = {
    data: [{ id: 'ent1', type: 'entry' }],
  ...
  const src = setupService('http://api14.test/database/{type}:{id}')
  const getService = (type, service) => (service === 'entries' ? src : null)
...
test('should allow several matches on one id when getting from member endpoints', async (t) => {
  nock('http://api6.test')
    .get('/entries/ent1')
    .reply(200, [
      { id: 'ent1', type: 'entry' },
  ...
  t.is(ret.status, 'ok', ret.error)
  t.true(Array.isArray(ret.data))
  t.is(ret.data.length, 4)
  t.is(ret.data[0].id, 'ent1')
  t.is(ret.data[1].id, 'ent1')
  t.is(ret.data[2].id, 'ent2')
  t.is(ret.data[3], undefined)
})

test('should filter away items with wrong id when filterWithId is true', async (t) => {
  nock('http://api1.test')
    .get('/database/entry:ent1')
    .reply(200, [
      { id: 'ent1', type: 'entry' },
      { id: 'ent1-1', type: 'entry' },
    ])
  const payload = {
    id: 'ent1',
    type: 'entry',
...
test('should not filter away items with wrong id when filterWithId is false', async (t) => {
  nock('http://api1.test')
    .get('/database/entry:ent1')
    .reply(200, [
      { id: 'ent1', type: 'entry' },
  ...
  t.is(ret.data.length, 2)
  t.is(ret.data[0].id, 'ent1')
  t.is(ret.data[1].id, 'ent1-1')
})

test('should filter away items with wrong id when several ids', async (t) => {
  nock('http://api12.test')
    .get('/entries')
    .query({ id: 'ent1,ent2' })
    .reply(200, [
