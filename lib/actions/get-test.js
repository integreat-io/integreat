...
function createService (uri, match = {}, {id = 'entries'} = {}) {
  const endpoints = [
    createEndpoint({match, options: {uri}}),
    createEndpoint({id: 'other', options: {uri: 'http://api5.test/other'}})
  ]
  return service({id, adapter: json, endpoints}, {schemas, mappings})
}
...
test('should get items by id array from service from member_s_ endpoint', async (t) => {
  ...
  const src = createService('http://api12.test/entries{?id}', {scope: 'members'})
  const getService = (type, service) => (service === 'entries') ? src : null
