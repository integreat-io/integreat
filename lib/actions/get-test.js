...
function createService (endpoint, match = {}, {id = 'entries'} = {}) {
  const endpoints = [
    createEndpoint({...match, uri: endpoint}),
    createEndpoint({id: 'other', uri: 'http://api5.test/other'})
  ]
  return service({id, adapter: json, endpoints}, {datatypes, mappings})
}
...
test('should get all items from service', async (t) => {
  ...
  const src = createService('http://api1.test/database')
  ...
  const ret = await get({payload, ident}, {getService, datatypes})
...
test('should get item by id from service', async (t) => {
  ...
  const src = createService('http://api1.test/database/{type}:{id}')
  const getService = (type, service) => (service === 'entries') ? src : null

  const ret = await get({payload}, {getService, datatypes})
...
test('should get items by id array from service from member_s_ endpoint', async (t) => {
  ...
  const src = createService('http://api1.test/entries{?id}', {scope: 'members'})
  const getService = (type, service) => (service === 'entries') ? src : null
...
test('should get items by id array from service from member endpoints', async (t) => {
  ...
  const src = createService('http://api6.test/entries/{id}', {scope: 'member'})
  const getService = (type, service) => (service === 'entries') ? src : null
...
test('should pass on ident when getting from id array', async (t) => {
  ...
  const ident = {id: 'johnf'}
  const src = createService('http://api11.test/entries/{id}', {scope: 'member'})
  sinon.stub(src, 'send').resolves({response: {status: 'ok', data: [{id: 'ent1', type: 'entry'}]}})
  const getService = (type, service) => src

  await get({payload, ident}, {getService, datatypes})
...
test('should return undefined for items not found when getting by id array', async (t) => {
  ...
  const src = createService('http://api10.test/entries/{id}', {scope: 'member'})
  const getService = (type, service) => (service === 'entries') ? src : null
...
test('should return error when one or more requests for individual ids fails', async (t) => {
  ...
  const src = createService('http://api8.test/entries/{id}', {scope: 'member'})
  const getService = (type, service) => (service === 'entries') ? src : null
...
test('should get item by id from service when id is array of one', async (t) => {
  ...
  const src = createService('http://api7.test/entries/{id}', {scope: 'member'})
  const getService = (type, service) => (service === 'entries') ? src : null
...
test('should get from other endpoint', async (t) => {
  ...
  const src = createService('http://api5.test/database')
  const getService = () => src
...
test('should get with uri params', async (t) => {
  ...
  const src = createService('http://api1.test/database{?first,max,type}')
  const getService = () => src
...
test('should return error on not found', async (t) => {
  ...
  const src = createService('http://api3.test/unknown')
  const getService = () => src
...
test('should return error if no payload', async (t) => {
  const payload = null
  const src = createService('http://api4.test/unknown')
  const getService = () => src
...
test('should get only authorized items', async (t) => {
  ...
  const ident = {id: 'johnf'}
  const src = createService('http://api9.test/database', {}, {id: 'accounts'})
