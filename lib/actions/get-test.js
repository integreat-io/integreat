...
test('should get items by id array from source from member_s_ endpoint', async (t) => {
  nock('http://api1.test')
    .get('/entries')
    .query({id: 'ent1,ent2'})
    .reply(200, [
...
test('should pass on ident when getting from id array', async (t) => {
  ...
  const ident = {id: 'johnf'}
  const src = createSource('http://api11.test/entries/{id}', {scope: 'member'})
  sinon.stub(src, 'send').resolves({response: {status: 'ok', data: [{id: 'ent1', type: 'entry'}]}})
  const getSource = (type, source) => src

  await get({payload, ident}, {getSource, datatypes})
