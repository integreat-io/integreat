...
async function sync ({ payload, meta = {} }, { dispatch, getService }) {
  ...
  const setMetaActions = createSetMetas(fromParams, lastSyncedAt, metaKey, meta.ident, dispatch, getService)
  const responses = await Promise.all([
    ...setMetaActions,
    ...createSetActions(toAction, data, toParams, { ...meta, queue: queueSet }, useIndividualSet).map(dispatch)
  ].map(p => limit(() => p)))
