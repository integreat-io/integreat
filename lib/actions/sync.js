...
const generateFromParams = async (
  ...
  if (retrieve === 'updated') {
    if (updatedAfter || updatedUntil) {
      return fromParams.map((params) => ({ ...params, updatedAfter, updatedUntil }))
    } else {
      return Promise.all(fromParams.map(setUpdatedParams(dispatch, ident)))
    }
  } else {
    return fromParams
  }
...
// should be used.
const generateToParams = ({ to }, fromParams) => {
...
const getFromService = (dispatch, type, ident) => async (fromParams) => {
  const response = await dispatch(action('GET', { type, ...fromParams }, { ident }))
...
async function sync ({ payload, meta = {} }, { dispatch }) {
  ...
  const results = await Promise.all(
    fromParams.map(getFromService(dispatch, type, ident))
  )
  ...
  return Promise.all([
    ...createSetMetas(fromParams, lastSyncedAt, ident, dispatch),
    dispatch(action('SET', { data, ...toParams }, { queue: true, ident }))
  ])
    .then((responses) => {
