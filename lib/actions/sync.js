...
const createSetMetas = (fromParams, lastSyncedAt, ident, dispatch, getService) => fromParams
...
async function sync ({ payload, meta = {} }, { dispatch, getService }) {
  ...
  return Promise.all([
    ...createSetMetas(fromParams, lastSyncedAt, meta.ident, dispatch, getService),
    dispatch(action('SET', { data, ...toParams }, { ...meta, queue: queueSet }))
  ])
    .then((responses) => {
