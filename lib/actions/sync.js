...
async function sync ({ payload, ident }, { dispatch }) {
  ...
  const response = await dispatch(action('GET', { service: from, type, params: fromParams }, { ident }))
  let { data } = response
  ...
  return Promise.all([
    ...
    dispatch(action('SET', { service: to, data, params: toParams }, { queue: true, ident }))
  ])
    .then((responses) => {
