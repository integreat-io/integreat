...
const generateUpdatedAfter = (updatedAfter) => ({
  updatedAfter,
  updatedSince: new Date(updatedAfter.getTime() + 1)
})

const setUpdatedParams = (dispatch, dontSetUntil, metaKey, ident) => async (params) => {
  ...
  if (status === 'ok' && data && data.meta && data.meta.lastSyncedAt) {
    return {
      ...params,
      ...generateUpdatedAfter(data.meta.lastSyncedAt),
      updatedUntil: dontSetUntil ? undefined : new Date()
    }
  } else {
...
const generateParamsWithUpdatedDates = async (params, dispatch, ident, updatedAfter, updatedUntil, metaKey) => {
  if (updatedAfter) {
    return params.map((params) => ({
      ...params,
      ...generateUpdatedAfter(new Date(updatedAfter)),
      updatedUntil: updatedUntil === 'open' ? undefined : updatedUntil ? new Date(updatedUntil) : new Date()
    }))
  } else {
    return Promise.all(params.map(setUpdatedParams(dispatch, updatedUntil === 'open', metaKey, ident)))
  }
}
...
const getLastSyncedAt = (fromParams, data, setLastSyncedAtFromData = false) =>
  setLastSyncedAtFromData
    ...
    : (fromParams[0] && fromParams[0].updatedUntil) || new Date() // TODO: Pick the latest date, not just the first
...
async function sync ({ payload, meta = {} }, { dispatch, getService }) {
  ...
  if (transformParams) {
    ...
    if (transformResponse.status === 'ok') {
      data = transformResponse.data
    }
  }

  const lastSyncedAt = getLastSyncedAt(fromParams, data, payload.setLastSyncedAtFromData)
