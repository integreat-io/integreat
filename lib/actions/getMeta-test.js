...
test('should get metadata for service', async (t) => {
  ...
  const ret = await getMeta(action, { getService })
...
test('should get metadata for service with metaKey', async (t) => {
  nock('http://api1.test')
    .get('/database/meta%3Astore%3Aproducts')
    .reply(200, { id: 'meta:store:products', _rev: '000001', type: 'meta', attributes: metadata })
  const endpoints = [{ options: { uri: 'http://api1.test/database/{id}' } }]
  const src = setupService('store', { meta: 'meta', endpoints })
  const getService = (type, service) => (service === 'store' || type === 'meta') ? src : null
  const action = {
