...
test('should dispatch GET to service', async (t) => {
  ...
  const expected = {
    ...
    meta: { ident, project: 'project1', cid: '12346', gid: '12345' },
  }
...
test.serial('should set lastSyncedAt on service', async (t) => {
  ...
  const action = {
    ...
    meta: { ident, id: '12345', cid: '12346', gid: '12347' },
  }
  const expected = {
    type: 'SET_META',
    payload: { service: 'users', meta: { lastSyncedAt } },
    meta: { ident, cid: '12346', gid: '12345' },
  }
...
test.serial('should set lastSyncedAt on service with metaKey', async (t) => {
  ...
  const expected = {
    type: 'SET_META',
    payload: { service: 'users', metaKey: 'products', meta: { lastSyncedAt } },
    meta: { ident, cid: '12346', gid: '12345' },
  }
        ...
        data: [
          {
            id: 'john',
            type: 'user',
            attributes: { updatedAt: new Date('2020-09-01T18:43:11Z') },
          },
        ],
      },
      SET: { status: 'queued' },
      ...
      GET: {
        ...
        data: [
          { id: 'lucyk', type: 'user', attributes: { updatedAt } },
          {
            id: 'john',
            type: 'user',
            attributes: { updatedAt: new Date('2020-09-01T11:13:09Z') },
          },
        ],
      },
      SET: { status: 'queued' },
...
test('should not set lastSyncedAt when there is no updates after date filter', async (t) => {
  ...
  const dispatch = sinon.spy(
    setupDispatch({
      ...
      GET: {
        status: 'ok',
        data: [{ id: 'john', type: 'user', attributes: { updatedAt } }],
      },
      SET: { status: 'queued' },
    })
  )
...
test('should pass on updatedSince', async (t) => {
  const lastSyncedAt = new Date('2017-05-13T18:43:00Z')
  const updatedAfter = new Date('2017-05-13T23:59:59.999Z')
  const updatedUntil = new Date('2017-05-14T23:59:59.999Z')
  const dispatch = sinon.spy(
  ...
  const expected = {
    ...
    payload: {
      ...
      type: 'user',
      updatedAfter,
      updatedSince: new Date(updatedAfter.getTime() + 1),
      updatedUntil,
    },
  }
  const notExpected = {
...
test('should adjust dates from afterDelta and untilDelta', async (t) => {
  const lastSyncedAt = new Date('2017-05-13T18:43:00Z')
  const updatedAfter = new Date('2017-05-13T23:59:59.999Z')
  const updatedUntil = new Date('2017-05-14T23:59:59.999Z')
  const dispatch = sinon.spy(
  ...
  const expected = {
    ...
    payload: {
      service: 'users',
      type: 'user',
      updatedAfter: new Date('2017-05-14T00:00:00.999Z'),
      updatedSince: new Date('2017-05-14T00:00:01.000Z'),
      updatedUntil: new Date('2017-05-14T23:59:59.998Z'),
    },
  }
  const notExpected = {
    type: 'GET_META',
...
test('should set lastSyncedAt on service without delta', async (t) => {
  const dispatch = sinon.spy(
    setupDispatch({
      GET: {
        status: 'ok',
        data: [
          {
            id: 'john',
            type: 'user',
            attributes: { updatedAt: new Date('2017-05-14T08:44:14') },
            relationships: {},
          },
        ],
      },
  ...
  const expectedGet = {
    ...
    payload: {
      ...
      type: 'user',
      updatedAfter: new Date('2017-05-14T00:00:00.999Z'),
      updatedUntil: new Date('2017-05-14T22:59:59.998Z'),
    },
    meta: { ident, cid: '12346', gid: '12345' },
  }
  const expectedSetMeta = {
    type: 'SET_META',
    payload: {
      service: 'users',
      meta: { lastSyncedAt: new Date('2017-05-14T23:59:59.998Z') },
    },
    meta: { ident, cid: '12346', gid: '12345' },
  }
  ...
  t.true(dispatch.calledWithMatch(expectedGet))
  t.true(dispatch.calledWithMatch(expectedSetMeta))
})
...
test('should not filter when noFilter is true', async (t) => {
  const updatedAfter = new Date('2017-05-13T23:59:59.999Z')
  const updatedUntil = new Date('2017-05-14T23:59:59.999Z')
  const date1 = new Date('2017-05-13T23:59:59.999Z')
  const date2 = new Date('2017-05-14T18:43:01Z')
  ...
  t.is(dispatch.args[2][0].type, 'SET')
  t.is(dispatch.args[2][0].payload.data.length, 3)
})
    ...
    setupDispatch({
      GET: {
        ...
        data: [
          {
            id: 'ent1',
            attributes: { updatedAt: new Date('2017-05-14T18:43:01Z') },
          },
        ],
      },
      SET: { status: 'queued' },
...
test('should set updatedUntil to now on SET action', async (t) => {
  ...
  t.true(
    dispatch.args[2][0].payload.updatedUntil < new Date(now.getTime() + 100)
  )
})
...
test('should run data through transform action', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  const jennyDataMod = {
    id: 'jenny',
    type: 'user',
    attributes: { name: 'Jenny', admin: true },
  }
  const dispatch = sinon.spy(
    setupDispatch({
      GET: { status: 'ok', data: [johnData, jennyData] },
      PREPARE_DATA: { status: 'ok', data: [johnData, jennyDataMod] },
      SET: { status: 'queued' },
    })
  )
  const action = {
  ...
  t.true(dispatch.callCount >= 4, `Called ${dispatch.callCount} times`)
...
test('should run data through several transform actions', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  const dispatch = sinon.spy(
    setupDispatch({
      GET: [
        { status: 'ok', data: [johnData, jennyData] },
        { status: 'ok', data: [johnData] },
        { status: 'ok', data: [jennyData] },
      ],
      PREPARE_DATA: [
        { status: 'ok', data: [johnData] },
        { status: 'ok', data: [jennyDataMod] },
      ],
      SET: { status: 'queued' },
    })
  )
  const action = {
    ...
    payload: {
      ...
      transform: [{ action: 'GET' }, { action: 'PREPARE_DATA' }],
      to: 'store',
      type: 'user',
      retrieve: 'all',
    },
  ...
  t.true(dispatch.callCount >= 7, `Called ${dispatch.callCount} times`)
  t.deepEqual(dispatch.args[1][0], expected1)
  t.deepEqual(dispatch.args[3][0], expected2)
  t.deepEqual(dispatch.args[6][0], expected6)
  t.is(ret.status, 'ok')
  t.true(Array.isArray(ret.data))
  t.is(ret.data.length, 1)
})

test('should not set when transform action returns no data', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  t.is(
    ret.error,
    "No items to update from service 'users' after transform action"
  )
  t.is(dispatch.callCount, 2)
})

test('should use specified payload for transform action', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  const dispatch = sinon.spy(
    setupDispatch({
      GET: { status: 'ok', data: [johnData, jennyData] },
      PREPARE_DATA: { status: 'ok', data: [johnData, jennyData] },
      SET: { status: 'queued' },
    })
  )
  const action = {
  ...
  t.true(dispatch.callCount >= 2, `Called ${dispatch.callCount} times`)
  t.deepEqual(dispatch.args[1][0], expected1)
  t.is(ret.status, 'ok')
})

test('should queue one transform per data item when useIndividualTransform is true', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  t.true(dispatch.callCount >= 5, `Called ${dispatch.callCount} times`)
  t.deepEqual(dispatch.args[1][0], expected1)
  t.deepEqual(dispatch.args[2][0], expected2)
  t.deepEqual(dispatch.args[4][0], expected4)
  t.is(ret.status, 'ok')
  t.true(Array.isArray(ret.data))
  t.is(ret.data.length, 1)
})

test('should return error when transform action fails', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = {
    id: 'jenny',
    type: 'user',
  ...
  const dispatch = sinon.spy(
    setupDispatch({
      GET: { status: 'ok', data: [johnData, jennyData] },
      PREPARE_DATA: { status: 'error', error: 'Oh, we messed up again' },
      SET: { status: 'queued' },
    })
  )
  const action = {
  ...
  t.is(ret.status, 'error', ret.error)
  t.is(ret.error, 'Oh, we messed up again')
  t.is(dispatch.callCount, 2)
})
