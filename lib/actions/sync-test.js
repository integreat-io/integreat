...
test('should run data through transform action', async (t) => {
  const johnData = { id: 'john', type: 'user', attributes: { name: 'John' } }
  const jennyData = { id: 'jenny', type: 'user', attributes: { name: 'Jenny' } }
  const jennyDataMod = { id: 'jenny', type: 'user', attributes: { name: 'Jenny', admin: true } }
  const dispatch = sinon.spy(setupDispatch({
    GET: { status: 'ok', data: [johnData, jennyData] },
...
test('should not set when transform action returns no data', async (t) => {
  ...
  t.is(ret.status, 'noaction')
  t.is(ret.error, 'No items to update from service \'users\' after transform action')
...
test('should queue one transform per data item when useIndividualTransform is true', async (t) => {
  ...
  const dispatch = sinon.spy(setupDispatch({
    GET: { status: 'ok', data: [johnData, jennyData] },
    PREPARE_DATA: [{ status: 'ok', data: [johnData] }, { status: 'ok', data: [jennyDataMod] }],
    SET: { status: 'queued' }
  }))
  const action = {
    type: 'SYNC',
