...
const setupAuth = (auths, liveAuths, getAuthStrategy) => {
  auths.forEach((authDef) => {
    const {strategy, options, id} = authDef
    const Strat = getAuthStrategy(strategy)
    const strat = new Strat(options)
    liveAuths.set(id, strat)
  })
}
...
class Integreat {
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      setupAuth(this._auths, this._liveAuths, this.getAuthStrategy.bind(this))
      ...
      this._sources.forEach((sourceDef) => {
