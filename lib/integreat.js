...
class Integreat {
   ...
   * If called with a sourceDef as only argument, sourcetype is used as id.
   * @param {string} id - Identifier for source definition
   * @param {Object} sourceDef - The source definition
   * @returns {void}
   */
  setSource (id, source) {
    if (typeof id === 'object' && !source) {
      // setSource(source) - get id from sourcetype
      rep.set(this._sources, id.sourcetype, id)
    } else {
      // setSource(id, source)
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      this._scheduleSubscription = this._scheduler.subscribe(processSource(this.getAdapter.bind(this), this._storage))
      resolve(this)
    })
  }
