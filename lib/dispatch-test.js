...
test('should call action handler with action', async (t) => {
  ...
  const action = {
    type: 'GET',
    payload: {},
    meta: { ident, id: '12345', cid: '12345', gid: '12345' },
  }
...
test('should call action handler with payload when not set', async (t) => {
  const getHandler = sinon.stub().resolves({ status: 'ok' })
  const actions = { GET: getHandler }
  const action = {
    type: 'GET',
    // No payload
    meta: { id: '12345', cid: '12345', gid: '12345' },
  }
  const expected = {
    type: 'GET',
    payload: {},
...
test('should generate id, gid, and cid when not set', async (t) => {
  const getHandler = sinon.stub().resolves({ status: 'ok' })
  const actions = { GET: getHandler }
  const ident = { id: 'ident1', roles: [], tokens: [] }
  ...
  t.is(dispatchedAction.meta.gid, dispatchedAction.meta.id)
})

test('should set gid, and cid from id when not set', async (t) => {
  const getHandler = sinon.stub().resolves({ status: 'ok' })
  const actions = { GET: getHandler }
  const ident = { id: 'ident1', roles: [], tokens: [] }
  const action = { type: 'GET', payload: {}, meta: { ident, id: '12345' } }
  const expectedMeta = { ident, id: '12345', cid: '12345', gid: '12345' }

  await dispatch({ actions })(action)

  t.is(getHandler.callCount, 1)
  const dispatchedAction = getHandler.args[0][0]
  t.deepEqual(dispatchedAction.meta, expectedMeta)
})
