const Dbdb = require('dbdb').couchdb
const storeItemDb = require('./storeItem')
const fetchItemDb = require('./fetchItem')
const fetchByTypeDb = require('./fetchByType')
const fetchSourceDefs = require('./fetchSourceDefs')

class Storage {
  constructor (config) {
    this._db = new Dbdb(config)
  }

  storeItem (item) {
    return storeItemDb(this._db, item)
  }

  storeItems (items) {
    // TODO: Bulk store instead of parallel storing single items
    return Promise.all(items.map((item) => storeItemDb(this._db, item)))
  }

  fetchItem (id, type) {
    return fetchItemDb(this._db, id, type)
  }

  fetchByType (type) {
    return fetchByTypeDb(this._db, type)
  }

  fetchSourceDefs () {
    return fetchSourceDefs(this._db)
  }
}

module.exports = Storage
