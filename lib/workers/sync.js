...
async function sync ({from, to, type, retrieve}, {dispatch, queue}) {
  ...
  const response = await dispatch(action('GET', {source: from, type, params}))
  let {data} = response

  if (response.status !== 'ok' || !Array.isArray(data)) {
    return createError(`Could not get items from source ${from}. Resons: ${response.status} ${response.error}`, 'noaction')
  }
