 ...
 */
async function deleteExpired ({source, type, endpoint, msFromNow = 0}, {dispatch, queue}) {
  const timestamp = Date.now() + msFromNow
  const isodate = new Date(timestamp).toISOString()
  const payload = {source, type, endpoint, params: {timestamp, isodate}}
  ...
  return queue(action('DELETE', {source, data: response.data}))
}
