...
test('should fetch source defs', async (t) => {
  const db = new Dbdb({})
  const sourceDefs = [
  ...
  const ret = await fetchSourceDefs(db)
...
test('should return empty array when none is found', async (t) => {
  const db = new Dbdb({})
