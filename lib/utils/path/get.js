const compare = (operator, value) => (candidate) => {
  switch (operator) {
    case '=':
...
const filter = ({path, operator, value}) => (item) => {
  const candidate = get(item, path)
  return (Array.isArray(candidate))
    ? candidate.some(compare(operator, value))
    : compare(operator, value)(candidate)
}
 ...
 * `alternative` property of the first path will be tried, then the next, etc.,
...
function get (object, path, defaultValue) {
  ...
  while (ret === undefined && path.alternative !== undefined) {
