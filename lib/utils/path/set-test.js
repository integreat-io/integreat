...
test('should return value when no object', (t) => {
  const path = ['meta']
  const value = 'Someone'
  const expected = 'Someone'

  const ret = set(null, path, value)
...
test('should set []', (t) => {
  const path = [{ prop: null, type: 'all', spread: true }]
  const value = [{ id: 'item1' }, { id: 'item2' }]
  const expected = [
    { id: 'item1' },
    { id: 'item2' }
...
test('should append to data.items[] on existing array with objects', (t) => {
  const path = ['data', { prop: 'items', type: 'all', spread: true }]
  const value = { id: 'item1' }
  const object = { data: { items: [{ id: 'item3' }] } }
  const expected = {
    data: {
      items: [
...
test('should append to [] on existing array with objects', (t) => {
  const path = [{ prop: null, type: 'all', spread: true }]
  const value = { id: 'item1' }
  const object = [{ id: 'item3' }]
  const expected = [
    { id: 'item3' },
    { id: 'item1' }
  ]
