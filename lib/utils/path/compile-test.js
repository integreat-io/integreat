...
test('should compile []', (t) => {
  const path = '[]'
  const expected = [{prop: null, type: 'all', spread: true}]
...
test('should compile data.item[keys]', (t) => {
  ...
  const expected = ['data', {prop: 'item', type: 'keys'}]
...
test('should compile data.item[values]', (t) => {
  ...
  const expected = ['data', {prop: 'item', type: 'values'}]
...
test('should compile meta.tags[]="news"', (t) => {
  ...
  const expected = {path: ['meta', {prop: 'tags', type: 'all', spread: true}], operator: '=', value: 'news'}
