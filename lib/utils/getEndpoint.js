...
const matchType = (endpoint, request) => !endpoint.type ||
  matchValue(endpoint.type, request.params && request.params.type)
const matchScope = (endpoint, {params = {}}) => !endpoint.scope ||
  matchValue(endpoint.scope, params.id ? (Array.isArray(params.id) ? 'members' : 'member') : 'collection')
