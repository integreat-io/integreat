...
const json = {
  ...
  prepareEndpoint (endpointOptions, sourceOptions = {}) {
    const {uri: uriTemplate, path: pathString, method = null} = endpointOptions
  ...
  async send (request, forceAuth = false) {
    const {endpoint, data, auth = null, method = 'PUT', headers = {}} = request

    if (!request.uri && !endpoint) {
      return createError('No uri or endpoint specified in the request')
    }

    const uri = request.uri || generateUri(endpoint.uri, {...request, ...request.params})
