...
const isAuthenticated = async (auth) => {
...
const hasArrEnding = (path) => (path.substr(-2) === '[]')
const removeArrEnding = (path) => path.substr(0, path.length - 2)
...
const json = {
  ...
  async retrieve (url, auth) {
    ...
    try {
      ...
      return {status: 'error', error: `Server returned error for ${url}: ${err}`}
    }
    ...
    if (status >= 200 && status < 300) {
      ...
      return {status: 'notfound', error: `Could not find the url ${url}`}
    } else {
      return {status: 'error', error: `Server returned ${response.statusCode} for ${url}`}
    }
  },
  ...
  async normalize (data, path = null) {
    ...
    if (hasArrEnding(path)) {
      path = removeArrEnding(path)
    }

    return getProp(data, path, null)
  },
