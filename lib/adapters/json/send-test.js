...
test('should add body properties from endpoint', async (t) => {
  ...
  const request = {endpoint, params: {id: ['ent1']}}
...
test('should return error on other error', async (t) => {
  nock('http://json2.test')
    .put('/entries/timeout', {})
    .reply(408)
  const request = {endpoint: prepareEndpoint({uri: 'http://json2.test/entries/timeout'}), data: {}}
...
test('should reject when authentication fails', async (t) => {
  ...
  const req = nock('http://json4.test')
    .put('/entries/ent1', {})
    .reply(200)
  const request = {endpoint: prepareEndpoint({uri: 'http://json4.test/entries/ent1'}), data: {}, auth}
...
test('should retrieve when authentication is successful', async (t) => {
  ...
  nock('http://json5.test')
    .put('/entries/ent1', {})
    .reply(200)
  const request = {endpoint: prepareEndpoint({uri: 'http://json5.test/entries/ent1'}), data: {}, auth}
...
test('should retrieve when already authenticated', async (t) => {
  ...
  nock('http://json6.test')
    .put('/entries/ent1', {})
    .reply(200)
  const request = {endpoint: prepareEndpoint({uri: 'http://json6.test/entries/ent1'}), data: {}, auth}
...
test('should retrieve with given headers', async (t) => {
  ...
  nock('http://json7.test', {
    ...
    .reply(200)
  const request = {
    endpoint: prepareEndpoint({uri: 'http://json7.test/entries/ent1'}),
    data: {},
...
test('should authenticate when request returns 401', async (t) => {
  ...
  const req = nock('http://json8.test')
    ...
    .put('/entries/ent1', {})
    .reply(200)
  const request = {endpoint: prepareEndpoint({uri: 'http://json8.test/entries/ent1'}), data: {}, auth}
...
test('should not authenticate on 401 when no auth', async (t) => {
  ...
  nock('http://json11.test')
    .put('/entries/ent1', {})
    .reply(401, {})
  const request = {endpoint: prepareEndpoint({uri: 'http://json11.test/entries/ent1'}), data: {}, auth}
