...
test('should return json object', (t) => {
  const json = {data: [
    {id: 'item1'},
    {id: 'item2'}
  ]}
...
test('should reject with error message on 404', (t) => {
  t.plan(2)
  nock('http://test.site')
    .get('/unknown')
    .reply(404)
...
test('should reject with error message on other error', (t) => {
  t.plan(2)
  nock('http://test.site')
    .get('/error')
...
test('should reject when authentication fails', (t) => {
  t.plan(3)
  const auth = {
  ...
  return retrieve('http://test.site/authfailed', auth)
...
test('should retrieve when authentication is successful', (t) => {
  const auth = {
    isAuthenticated: () => false,
  ...
  return retrieve('http://test.site/authsuccess', auth)
...
test('should retrieve when already authenticated', (t) => {
  const auth = {
...
test('should retrieve with auth headers', (t) => {
  const auth = {
    isAuthenticated: () => true,
  ...
  return retrieve('http://test.site/authorize', auth)
