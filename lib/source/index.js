...
class Source {
   ...
   */
  async normalizeMapFilter (data, itemDef) {
    if (!this.adapter || !data || !itemDef) {
      return []
    }

    const items = await this.adapter.normalize(data, itemDef.path)
    return items
      .map((item) => itemDef.mapItem(item))
      .filter((item) => itemDef.filterItem(item))
  }
  ...
  async fetchItems () {
    ...
    const arrays = await Promise.all(this.items.map((itemDef) => this.normalizeMapFilter(data, itemDef)))
