...
const castData = (request, datatypes, useDefaults) => {
  const castOne = (item) => (datatypes[item.type]) ? datatypes[item.type].cast(item, {useDefaults}) : undefined
...
const authorizeRequestAndData = (request, datatypes) => {
  const req = authorizeRequest(request, datatypes)
...
function source (
  ...
  return {
    ...
    async send (request, {useDefaults = false, unmapped = false} = {}) {
      ...
      const castedRequest = castData(preparedRequest, datatypes, useDefaults)
      const authorizedRequest = authorizeRequestAndData(castedRequest, datatypes)

      const {params, data} = authorizedRequest
      const toData = mapToSource(data, mappings)

      let response = await sendRequest({...authorizedRequest, data: toData}, {adapter, sourceId})
      ...
      const preparedResponse = prepareResponse(response, authorizedRequest, {datatypes, unmapped})
