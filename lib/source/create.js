...
module.exports = function create (sourceDef, getAdapter, getMapper, getFilter, getAuth) {
  ...
  if (sourceDef.items) {
    ...
    if (typeof itemDef.filter === 'object' && !Array.isArray(itemDef.filter)) {
      setPipeline(item.filters.from, itemDef.filter.from, getFilter)
      setPipeline(item.filters.to, itemDef.filter.to, getFilter)
    } else {
      setPipeline(item.filters.from, itemDef.filter, getFilter)
    }
    ...
    if (attrDefs) {
      Object.keys(attrDefs).forEach((key) => {
        ...
        if (attrDef) {
          ...
          const attr = new Attribute(key, type, attrDef.path, attrDef.defaultFrom, attrDef.defaultTo)
