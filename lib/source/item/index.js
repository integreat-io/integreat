...
class Item {
   ...
   * @param {(Function[]|Object[])} map - A map pipeline
   * @param {(Function[]|Object[])} filter - A filter pipeline
   */
  constructor (type = Item.defaultType, path = null, map = [], filter = []) {
    this.type = type
    this.path = path
    this._map = map
    this._filter = filter
  ...
  mapItem (source) {
    ...
    this.attributes.forEach((attr) => {
      const value = retrieveFromPath(source, attr.path)
      targetAttrs[attr.key] = attr.mapAttribute(value)
    })

    const targetRels = {}
    this.relationships.forEach((rel) => {
      const value = retrieveFromPath(source, rel.path)
