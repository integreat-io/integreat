...
class Item {
   ...
   * Will use from pipeline by default. Call with reverse = true to use to
   * pipeline.
   * @param {Object} source - The source item to filter
   * @param {boolean} reverse - True to filter to source
   * @returns {boolean} True if source passed all filters
   */
  filterItem (source, reverse = false) {
    return filterWithFilters(source, (reverse) ? this.filters.to : this.filters.from)
  }
}
