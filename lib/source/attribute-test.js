...
test('should set key, type, path, defaultFrom, and defaultTo', (t) => {
  const attr = new Attribute('id', 'integer', 'some.path', 'from', 'to')

  t.is(attr.key, 'id')
  t.is(attr.type, 'integer')
  t.is(attr.path, 'some.path')
  t.is(attr.defaultFrom, 'from')
  t.is(attr.defaultTo, 'to')
})
...
test('should set default attributes', (t) => {
  ...
  t.is(attr.path, null)
  t.is(attr.defaultFrom, null)
  t.is(attr.defaultTo, null)
...
test('should accept 0 as a default value', (t) => {
  ...
  t.is(attr.defaultFrom, 0)
  t.is(attr.defaultTo, 0)
})
