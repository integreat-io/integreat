...
test('send should return complete request', async (t) => {
  ...
  const ret = await source.send(request)
...
test('send should retrieve from endpoint with default values', async (t) => {
  ...
  const {response} = await source.send(request, {useDefaults: true})
...
test('send should cast, map and send data to source', async (t) => {
...
test('send should skip unknown datatypes', async (t) => {
  const send = sinon.stub().resolves({status: 'ok', data: []})
  const adapter = {...json, send}
  const source = setupSource({id: 'entries', adapter}, {mappings})
  const request = {
  ...
  const {request: req} = await source.send(request)
...
test('send should remove unauthorized data items in request', async (t) => {
  const send = sinon.stub().resolves({status: 'ok', data: []})
  const adapter = {...json, send}
  const source = setupSource({id: 'accounts', adapter}, {mappings})
  const request = {
    action: 'SET',
    data: [
      {id: 'johnf', type: 'account'},
