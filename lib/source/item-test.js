...
test('filterItem should filter through pipeline from source', (t) => {
  const item = new Item('entry')
...
test('filterItem should filter through pipeline to source', (t) => {
  const item = new Item('entry')
  item.filters.to.push((item) => true)
  item.filters.to.push((item) => false)
  const source = {}

  const ret = item.filterItem(source, true)
