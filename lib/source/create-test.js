...
test('should set attribute mapper', (t) => {
  ...
  const map1 = ret.items[0].attributes[0].map
  t.is(map1.length, 2)
...
test('should add date mapper when defaulting to date type', (t) => {
  ...
  const sourceDef = {id: 'entry1', items: [{attributes}]}
  const getMapper = (key) => (key === 'date') ? () => 'A date' : null

  const ret = create(sourceDef, null, getMapper)

  const map = ret.items[0].attributes[0].map
  t.is(map.length, 1)
...
test('should map arrays', (t) => {
  ...
  const ret = create(sourceDef, null, getMapper)
  const attr = ret.items[0].attributes[0]
  const target = attr.mapAttribute(source)
