...
test('should create Item', (t) => {
  ...
  t.is(ret.item.constructor.name, 'Item')
  t.is(ret.item.type, 'entry')
  t.is(ret.item.path, 'the.path')
})
...
test('should set attributes', (t) => {
  ...
  const attrs = ret.item.attributes
  t.is(attrs.length, 2)
  const attr1 = attrs[0]
...
test('should not set attribute with no def', (t) => {
  ...
  t.is(ret.item.attributes.length, 0)
})
...
test('should set attribute mapper', (t) => {
  ...
  const attr1 = ret.item.attributes[0]
