...
test('should have defaultType', (t) => {
  t.is(ItemMapper.defaultType, 'unset')
})

test('should set default values', (t) => {
  const item = new ItemMapper()

  t.is(item.type, ItemMapper.defaultType)
  t.is(item.path, null)
  t.deepEqual(item.map, [])
  t.deepEqual(item.filters.from, [])
  t.deepEqual(item.filters.to, [])
  t.deepEqual(item.attrMappers, [])
  t.deepEqual(item.relMappers, [])
})
...
test('fromSource should map attributes', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.attrMappers.push(new ValueMapper('one', 'integer', 'values.first'))
  item.attrMappers.push(new ValueMapper('two', 'integer', 'values.second.value'))
...
test('fromSource should use id attribute as id for item', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.attrMappers.push(new ValueMapper('id', 'string', 'key'))
...
test('fromSource should not use source type as type for item', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.attrMappers.push(new ValueMapper('type', 'string', 'type'))
...
test('fromSource should use createAt and updatedAt attributes as dates for item', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.attrMappers.push(new ValueMapper('createdAt', 'date', 'createdAt'))
  item.attrMappers.push(new ValueMapper('updatedAt', 'date', 'updatedAt'))
...
test('fromSource should set relationships', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.relMappers.push(new ValueMapper('comments', 'comment', 'item.note'))
...
test('fromSource should set relationship with array', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.relMappers.push(new ValueMapper('comments', 'comment', 'item.notes'))
...
test('toSource should transform attributes', (t) => {
  ...
  const item = new ItemMapper('entry')
  const attr = new ValueMapper('one', 'integer', 'values.first')
  attr.map.push({to: (value) => value + 2})
  item.attrMappers.push(attr)
...
test('toSource should map createdAt and updatedAt', (t) => {
  ...
  const item = new ItemMapper('entry')
  item.attrMappers.push(new ValueMapper('createdAt', 'date', 'createdDate'))
  item.attrMappers.push(new ValueMapper('updatedAt', 'date', 'updatedDate'))
...
test('toSource should transform relationships', (t) => {
  const data = {id: 'ent1', type: 'entry', relationships: {comments: {id: 'no1', type: 'comment'}}}
  const item = new ItemMapper('entry')
  const rel = new ValueMapper('comments', 'comment', 'item.note')
  rel.map.push({to: (value) => 'com_' + value})
  item.relMappers.push(rel)
