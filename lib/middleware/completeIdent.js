...
const completeIdent = (next) => async (action) => {
  ...
  // Pass on to next middleware when no ident or nothing to fetch
  if (!meta.ident || !(meta.ident.id || meta.ident.withToken)) {
    return next(action)
  }
  ...
  if (response.status === 'ok') {
    ...
    const ident = response.access && response.access.ident
