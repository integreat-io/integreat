...
const createSource = require('./integreat/createSource')
const fetchSourceDefs = require('./utils/fetchSourceDefs')
const router = require('./integreat/router')
...
class Integreat {
  ...
  async dispatch (action) {
    return await router(action, this.sources)
  }
  ...
  async _syncSource (source) {
    ...
    try {
      const items = await processSource(source, this.dispatch.bind(this))
      scheduleSource(this._scheduler, source)
