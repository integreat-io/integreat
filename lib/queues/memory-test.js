...
test('push should queue action and dispatch it', (t) => {
  ...
  const queue = memory(null, {altScheduler: ts})
...
// Tests -- bind to integreat

test('should set queue method', (t) => {
  const great = {queue: null}

  const queue = memory(great)

  t.is(great.queue, queue.push)
})

test('should subscribe with dispatch', (t) => {
  const ts = new Rx.TestScheduler()
  const replay = new Rx.ReplaySubject()
  const dispatch = sinon.stub().returns({status: 'queued'})
  const great = {dispatch}
  const action = {}

  const queue = memory(great, {altScheduler: ts})
  ts.startScheduler(() => {
    queue.push(action)
    return replay
  })
  ...
  t.true(dispatch.calledOnce)
  t.true(dispatch.calledOn(great))
  t.true(dispatch.calledWith(action))
})

// Tests -- end

test('end should exist', (t) => {
  const queue = memory()

  t.is(typeof queue.end, 'function')
})

test('end should unsubscribe integreat', (t) => {
  const ts = new Rx.TestScheduler()
  const replay = new Rx.ReplaySubject()
  const dispatch = sinon.stub().returns({status: 'queued'})
  const great = {dispatch}
