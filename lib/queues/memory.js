 ...
 */
function memory (dispatch, options = {}) {
  const {altScheduler} = options
  const subject = new Rx.Subject()
  const queue = subject.map(timePayload(altScheduler)).mergeAll()
  queue.subscribe((action) => { dispatch(action) })
  ...
  return {
     ...
     * @returns {void}
     */
    async push (action, timestamp = Date.now()) {
      debug('Queued for %s: %o', new Date(timestamp), action)
      subject.onNext([timestamp, action])
      return true
    },
