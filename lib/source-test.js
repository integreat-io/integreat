...
test('prepareRequest should set typePlural from plurals dictionary', (t) => {
  ...
  const src = source({id: 'accounts', endpoints, adapter: json}, {plurals})
...
test('send should return casted data', async (t) => {
  ...
  const request = src.prepareRequest({params, data, method: 'POST'})
  sinon.stub(src, 'sendRaw').resolves({status: 'ok', data: {}})
  const expected = {
    status: 'ok',
    data: [{
...
test('send should return error when send fails', async (t) => {
  const def = {
    id: 'entries',
    endpoints: [createEndpoint({uri: 'http://some.api/1.0'})],
    adapter: json
  ...
  const src = source(def, {mappings})
  const request = src.prepareRequest({data: {}, method: 'POST'})
  sinon.stub(src, 'sendRaw').resolves({status: 'error', error: 'Big mistake!'})
  const expected = {
    status: 'error',
    error: 'Big mistake!'
  }
