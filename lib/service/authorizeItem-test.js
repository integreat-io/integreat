...
test('should authorize for specified method', (t) => {
  const access = { access: 'none', methods: { QUERY: { roleFromField: 'role' } } }
  const schemas = { entry: { id: 'entry', access } }
  const ident = { id: 'ident1', roles: ['admin'] }
  const data = createItem('ent1', 'entry', { role: 'admin' })

  const ret = authorizeItem(data, { ident }, { schemas, method: 'QUERY' })
