...
test('should just return when response set and has error', async (t) => {
  const request = {
    action: 'GET',
    data: [{ id: 'ent1', type: 'entry' }],
    endpoint: { uri: ['http://some.api/1.0'], path: ['item'] },
...
test('should still serialize when response is ok', async (t) => {
  const request = {
    action: 'GET',
    data: [{ id: 'ent1', type: 'entry' }],
    endpoint: { uri: ['http://some.api/1.0'], path: ['item'] },
...
test('should still serialize when response has no status', async (t) => {
  const request = {
    action: 'GET',
    data: [{ id: 'ent1', type: 'entry' }],
    endpoint: { uri: ['http://some.api/1.0'], path: ['item'] },
