...
test('should use status code and error from mapping', (t) => {
  ...
  const endpoint = { responseMapper: ({ data }) => data }

  const ret = mapFromService({ mappings })({ response, request, endpoint })
...
test('should map data without returning status code', (t) => {
  ...
  const endpoint = { responseMapper: ({ data }) => ({ data: data.data }) }
...
test('should not map and cast data from mutation when no responseMapping', (t) => {
  const endpoint = {}
  const request = {
    method: 'MUTATION',
    params: { service: 'thenews', type: 'entry' },
