...
test('should return data on error', (t) => {
  const request = {
    action: 'GET',
    params: { service: 'thenews', type: 'entry' },
  }
...
test('should treat as error when mapping results in mapping field only', (t) => {
  const request = {
    action: 'GET',
    params: { service: 'thenews', type: 'entry' },
    access: { ident: { id: 'johnf' } },
...
test('should use original status code and append to error on error response', (t) => {
  const request = {
    action: 'GET',
    params: { service: 'thenews', type: 'entry' },
    access: { ident: { id: 'johnf' } },
  ...
  t.is(ret.error, 'Not found. | Well. It failed')
  t.deepEqual(ret.access, { ident: { id: 'johnf' } })
})
