...
test('receive should dispatch action with mapped data', async (t) => {
  const dispatch = sinon.stub().resolves({ status: 'ok', data: [] })
  const service = setupService({ setupMapping, schemas })({
    id: 'entries',
    endpoints: [{
  ...
  const action = {
    type: 'REQUEST',
    payload: { data: '{"items":[{"key":"ent1","header":"Entry 1"}]}' },
    meta: { ident: { id: 'johnf' } }
  }
  const expected = {
    type: 'SET',
...
test('receive should respond with error when no endpoint matches', async (t) => {
  const dispatch = sinon.stub().resolves({ status: 'ok', data: [] })
  const service = setupService({ setupMapping, schemas })({
    id: 'entries',
    adapter: json
