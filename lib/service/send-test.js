...
test('send should retrieve and map data from endpoint', async (t) => {
  ...
  const endpoint = {
    responseMapper: (response) => response.data.content,
    options: { uri: 'http://some.api/1.0' }
  }
  const request = {
    method: 'QUERY',
...
test('send should return authorized request data before mapping', async (t) => {
  const adapter = createAdapter()
  const request = {
    method: 'MUTATION',
    params: {},
...
test('should respond with noaction for unknown action type', async (t) => {
  ...
  const expectedResponse = {
    status: 'noaction',
    access: { status: 'granted', ident: null, scheme: null }
  }
