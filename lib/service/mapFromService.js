...
const mapWithEndpoint = (endpoint = {}, response, method) => {
  if (method === 'QUERY' || endpoint.responseMapping) {
    return (endpoint.responseMapper) ? endpoint.responseMapper(response) : response
  } else {
    return {}
  }
}
...
function mapFromService ({ mappings }) {
  return ({ response, request, endpoint }) => {
    ...
    const { data, status = response.status, error } =
      mapWithEndpoint(endpoint, response, method)
