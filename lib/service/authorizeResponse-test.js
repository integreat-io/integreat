...
test('should not authorize data when mapResponseWithType is false', (t) => {
  const schemas = { user: { id: 'user', access: { role: 'admin' } } }
  const access = { status: 'granted', scheme: 'auth', ident: { id: 'johnf' } }
  const request = { action: 'GET', params: { type: 'user' }, access }
  const data = [{ id: 'johnf', type: 'user' }, { id: 'betty', type: 'user' }]
  ...
  const expected = {
    status: 'ok',
    data: [{ id: 'johnf', type: 'user' }, { id: 'betty', type: 'user' }],
    access: { status: 'granted', scheme: 'unmapped', ident: { id: 'johnf' } }
  }
  const mapResponseWithType = false

  const ret = authorizeResponse({ schemas })({ response, request, mapResponseWithType })
