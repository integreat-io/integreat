...
const castData = (request, schemas) => {
  ...
  return {
    ...
    async send (action, { unmapped = false } = {}) {
      ...
      if (response.data && !unmapped) {
        response = mapFromService(response, { mappings, request, endpoint })
      }
