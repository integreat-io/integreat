...
function service (
  ...
  // Prepare endpoints
  const prepareEndpoint = (endpointOptions) => adapter.prepareEndpoint(endpointOptions, serviceOptions)
  endpoints = endpoints
    .map((endpoint) => ({
      ...endpoint,
  ...
  return {
    ...
    async send (request, {useDefaults = false, unmapped = false} = {}) {
      const preparedRequest = prepareRequest(request, {auth, endpoints, schemas, useDefaults})
