...
const normalizeAction = async (action, adapter) => {
  const normalized = await adapter.normalize({ data: action.payload.data }, {})
...
const receiveRequestFromAction = ({ type, payload: { data, ...params }, meta: { ident } }, endpoint) =>
  ({ method: 'QUERY', action: type, params, endpoint: endpoint.options, access: { ident } })
...
const service = ({
  ...
  return {
    ...
    async send (action) {
      ...
      if (!endpoint) {
        return { response: createError(`No endpoint matching request to service '${serviceId}'.`) }
      }
