...
const castData = (request, schemas, onlyMappedValues) => {
  const castOne = (item) => (schemas[item.type]) ? schemas[item.type].cast(item, { onlyMappedValues }) : undefined
...
const service = ({
  ...
  return {
     ...
     */
    async send (action, { onlyMappedValues = true, unmapped = false } = {}) {
      const endpoint = getEndpoint(endpoints, action)
      let request = prepareRequest(action, { auth, schemas, endpoint })
      request = castData(request, schemas, onlyMappedValues)
      ...
      if (response.data && !unmapped) {
        response = mapFromService(response, { mappings, request, onlyMappedValues, endpoint })
      }
