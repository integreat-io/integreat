...
const mapDataPerType = (typeArrays, request, mappings) => Object.keys(typeArrays).reduce(
  (target, type) => mapItems(typeArrays[type], request, mappings[type], target),
  undefined
)

const mapData = (data, request, mappings) => (data)
  ? (Array.isArray(data))
    ? mapDataPerType(groupByType(data), request, mappings)
    : mapItems(data, request, mappings[data.type])
  : undefined

const applyEndpointMapper = (data, request, requestMapper) => (requestMapper)
  ? requestMapper({ ...request, data })
  : data

const extractFromData = ({ headers, ...data }) => ({
  data,
  ...(headers ? { headers } : {})
})

const deconstruct = (data) => typeof data === 'object' && data !== null && !Array.isArray(data)
  ? extractFromData(data) : { data }
...
function mapToService () {
  return ({ request, requestMapper, mappings }) => {
    ...
    return {
      ...request,
      ...deconstruct(applyEndpointMapper(data, request, requestMapper))
    }
  }
}
