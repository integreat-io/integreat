...
const authenticator = {
  ...
  asHttpHeaders: ({ token }) => ({ Authorization: token }),
  asObject: ({ token }) => ({ token }),
}
...
test('should override adapter method', async (t) => {
  const authOptions = { token: 't0k3n', overrideAdapterMethod: 'asObject' }
  const expectedAuth = { token: 't0k3n' }
...
test('should call authenticator.authenticate with request', async (t) => {
  const requestAuthenticator = {
    ...authenticator,
    authenticate: async (_authOptions, request) => ({
      status: 'granted',
      expired: false,
      token: request.access.ident.id,
    }),
  }
  const authOptions = {}
