...
test('should map data', (t) => {
  ...
  const ret = mapToService({ mappings })({ request })
...
test('should map array of data', (t) => {
  ...
  const ret = mapToService({ mappings })({ request: { data } })
...
test('should map with request mapper', (t) => {
  ...
  const endpoint = {
    requestMapper: createRequestMapper({ requestMapping: 'content' })
  }
  const request = {
    method: 'MUTATION',
    params: { service: 'thenews' },
  ...
  const ret = mapToService({ mappings })({ request, endpoint })
