...
test('should grant request for identFromField with data', (t) => {
  ...
  const request = {
    action: 'GET',
    params: { type: 'entry' },
    data: [{ id: 'ent1', attributes: { author: 'ident1' } }],
    access: { ident: { id: 'ident1' } }
  }
  const expectedAccess = {
    status: 'granted',
...
test('should grant request for identFromField for some data', (t) => {
  ...
  const request = {
    ...
    params: { type: 'entry' },
    data: [
      { id: 'ent1', type: 'entry', attributes: { author: 'ident1' }, relationships: {} },
      { id: 'ent2', type: 'entry', attributes: { author: 'ident2' }, relationships: {} }
    ],
    access: { ident: { id: 'ident1' } }
  }
  const expectedAccess = {
