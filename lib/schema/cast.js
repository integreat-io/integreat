...
const getRelId = (rel) => (rel.id) ? rel.id : rel
const castRel = (type) => (val) => ({ id: getRelId(val).toString(), type })
const castRels = (val, type) => (val === null) ? null : mapAny(castRel(type), getRelId(val))
